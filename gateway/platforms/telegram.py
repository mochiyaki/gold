...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("GOLD_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = float(os.getenv("GOLD_TELEGRAM_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("GOLD_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        message = (
            ...
            "(possibly OpenClaw or another Gold instance). "
            "Gold stopped Telegram polling after %d retries. "
            "Only one poller can run per token — stop the other process "
            "and restart with 'gold start'."
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("GOLD_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("GOLD_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("GOLD_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("GOLD_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("GOLD_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            proxy_url = resolve_proxy_url()
            disable_fallback = (os.getenv("GOLD_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in ("1", "true", "yes", "on"))
            ...
            try:
                ...
                from gold_cli.commands import telegram_menu_commands
