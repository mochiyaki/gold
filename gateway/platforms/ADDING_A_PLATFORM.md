...
Checklist for integrating a new messaging platform into the Gold gateway.
...
And add it to the `gold-gateway` composite:

```python
"gold-gateway": {
    "includes": [..., "gold-your-platform"]
}
```
...
## 12. Status Display (`gold_cli/status.py`)
...
## 13. Gateway Setup Wizard (`gold_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ gold_cli/ toolsets.py \
  --include="*.py" -l | sort -u
