...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Gold-Session-Id header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists gold as an available model
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to gold
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_gold_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        try:
...
def _derive_chat_session_id(
    ...
    the same Gold session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through gold's AIAgent.
    """
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``gold sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the gold-api-server default.
        """
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "gold"})
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return gold as an available model."""
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # Allow caller to continue an existing session by passing X-Gold-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Gold-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Gold-Session-Id rejected: "
            ...
            # conversation map to the same Gold session.  The first user
        ...
        if stream:
            ...
            def _on_tool_progress(event_type, name, preview, args, **kwargs):
                ...
                ``event: gold.tool.progress`` line that compliant frontends
        ...
        return web.json_response(response_data, headers={"X-Gold-Session-Id": session_id})
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Gold-Session-Id"] = session_id
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: gold.tool.progress`` SSE event so
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from gold_cli.auth import has_usable_secret
