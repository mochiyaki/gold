...
The PID file lives at ``{GOLD_HOME}/gateway.pid``.  GOLD_HOME defaults to
``~/.gold`` but can be overridden via the environment variable.  This means
separate GOLD_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting GOLD_HOME."""
    home = get_gold_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("GOLD_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "gold" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Gold gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different GOLD_HOME dirs).
    """
