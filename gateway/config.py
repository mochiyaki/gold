...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_gold_home() / "sessions")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        sessions_dir = get_gold_home() / "sessions"
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.gold/config.yaml (primary user-facing config)
    3. ~/.gold/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_gold_home()
    gw_data: dict = {}
