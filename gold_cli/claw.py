"""gold claw — OpenClaw migration commands.
...
Usage:
    ...
    gold claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    gold claw cleanup              # Archive leftover OpenClaw directories
    gold claw cleanup --dry-run    # Preview what would be archived
"""
...
from gold_cli.config import get_gold_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Gold may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Gold gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Gold gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'gold stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_gold.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_gold", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: gold claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Gold")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'gold claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Gold migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: gold claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: gold claw migrate --yes")
            return
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: gold claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: gold claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  gold claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  gold claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  gold config set OPENROUTER_API_KEY sk-or-v1-...")
