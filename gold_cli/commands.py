"""Slash command definitions and autocomplete for the Gold CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Gold config/state", "Session",
    ...
    CommandDef("update", "Update Gold Agent to the latest version", "Info",
               gateway_only=True),
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``gold skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /gold handler.

    Maps both canonical names and aliases so /gold bg do stuff works
    the same as /gold background do stuff.
    """
    overrides = _resolve_config_gates()
