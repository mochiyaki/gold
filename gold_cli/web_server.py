"""
Gold Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m gold_cli.main web          # Start on http://127.0.0.1:9119
    python -m gold_cli.main web --port 8080
"""
...
try:
    ...
    raise SystemExit(
        ...
        "Run 'gold web' to auto-install, or: pip install gold[web]"
    )

WEB_DIST = Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Gold Agent", version=__version__)
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Gold supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# the canonical ``gold auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Gold resolves Anthropic creds in this order at runtime:
    1. ``~/.gold/.anthropic_oauth.json`` — Gold-managed PKCE flow
    ...
    try:
        ...
        read_gold_oauth_credentials = None  # type: ignore
        _GOLD_OAUTH_FILE = None  # type: ignore

    gold_creds = None
    if read_gold_oauth_credentials:
        try:
            gold_creds = read_gold_oauth_credentials()
        except Exception:
            gold_creds = None
    if gold_creds and gold_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "gold_pkce",
            "source_label": f"Gold PKCE ({_GOLD_OAUTH_FILE})",
            "token_preview": _truncate_token(gold_creds.get("accessToken")),
            "expires_at": gold_creds.get("expiresAt"),
            "has_refresh_token": bool(gold_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Gold even
    when they also have a separate Gold-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "gold auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "gold auth add nous",
        "docs_url": "https://portal.mochiyaki.com",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "gold auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "gold auth add qwen-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from gold_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("gold_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Gold-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in ("anthropic", "claude-code"):
        try:
            from agent.anthropic_adapter import _GOLD_OAUTH_FILE
            if _GOLD_OAUTH_FILE.exists():
                _GOLD_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from gold_cli.auth import clear_provider_auth
...
#          → persists to ~/.gold/.anthropic_oauth.json AND credential pool
...
# Guarded so gold web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Gold file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``gold auth add anthropic``.
    """
    from agent.anthropic_adapter import _GOLD_OAUTH_FILE
    payload = {
    ...
    _GOLD_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _GOLD_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    from gold_cli import auth as hauth
    if provider_id == "nous":
        from gold_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("GOLD_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from gold_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
...
async def get_logs(
    ...
    from gold_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_gold_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from gold_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def get_skills():
    ...
    from gold_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from gold_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from gold_cli.tools_config import (
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index():
        ...
        token_script = (
            f'<script>window.__GOLD_SESSION_TOKEN__="{_SESSION_TOKEN}";</script>'
        )
...
def start_server(
    ...
    print(f"  Gold Web UI → http://{host}:{port}")
