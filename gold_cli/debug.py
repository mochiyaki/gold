"""``gold debug`` — debug tools for Gold Agent.

Currently supports:
    gold debug share    Upload debug report (system info + logs) to a
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----GoldDebugBoundary9f3c"
...
def _resolve_log_path(log_name: str) -> Optional[Path]:
    ...
    """
    from gold_cli.logs import LOG_FILES
...
def _read_log_tail(log_name: str, num_lines: int) -> str:
    ...
    from gold_cli.logs import _read_last_n_lines
...
def _capture_dump() -> str:
    """Run ``gold dump`` and return its stdout as a string."""
...
def collect_debug_report(*, log_lines: int = 200, dump_text: str = "") -> str:
    ...
    dump_text
        Pre-captured dump output.  If empty, ``gold dump`` is run
        internally.
...
def run_debug(args):
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: gold debug share [--lines N] [--expire N] [--local]")
        print()
        print("Commands:")
