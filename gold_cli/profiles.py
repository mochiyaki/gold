"""
Profile management for multiple isolated Gold instances.

Each profile is a fully independent GOLD_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.gold/profiles/<name>/`` by default.

The "default" profile is ``~/.gold`` itself — backward compatible,
zero migration needed.

Usage::

    gold profile create coder          # fresh profile + bundled skills
    gold profile create coder --clone  # also copy config, .env, SOUL.md
    gold profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    gold -p coder chat                 # or via flag
    gold profile use coder             # set as sticky default
    gold profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See gold_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.gold) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "gold",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "gold", "default", "test", "tmp", "root", "sudo",
})

# Gold subcommands that cannot be used as profile names/aliases
_GOLD_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the gold root, NOT to the current GOLD_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where GOLD_HOME points outside
    ``~/.gold``, profiles live under ``GOLD_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_gold_home() / "profiles"


def _get_default_gold_home() -> Path:
    """Return the default (pre-profile) GOLD_HOME path.

    In standard deployments this is ``~/.gold``.
    In Docker/custom deployments where GOLD_HOME is outside ``~/.gold``
    (e.g. ``/opt/data``), returns GOLD_HOME directly.
    """
    from gold_constants import get_default_gold_root
    return get_default_gold_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_gold_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its GOLD_HOME directory."""
    if name == "default":
        return _get_default_gold_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, gold subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _GOLD_SUBCOMMANDS:
        return f"'{name}' conflicts with a gold subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_gold_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.gold)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches GOLD_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "GOLD_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.gold).\n"
            "To remove everything, use: gold uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set GOLD_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("GOLD_HOME")
    try:
        os.environ["GOLD_HOME"] = str(profile_dir)
        from gold_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["GOLD_HOME"] = old_home
        elif "GOLD_HOME" in os.environ:
            del os.environ["GOLD_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.gold/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: gold profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from GOLD_HOME.

    Returns ``"default"`` if GOLD_HOME is not set or points to ``~/.gold``.
    Returns the profile name if GOLD_HOME points into ``~/.gold/profiles/<name>``.
    Returns ``"custom"`` if GOLD_HOME is set to an unrecognized path.
    """
    from gold_constants import get_gold_home
    gold_home = get_gold_home()
    resolved = gold_home.resolve()

    default_resolved = _get_default_gold_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.gold itself — its parent is ~/ and its
        # directory name is ".gold", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: gold profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.gold itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.gold). "
            "Specify a different name: gold profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for gold profile names."""
    return '''# Gold Agent profile completion
# Add to ~/.bashrc: eval "$(gold completion bash)"

_gold_profiles() {
    local profiles_dir="$HOME/.gold/profiles"
...
_gold_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_gold_profiles)" -- "$cur"))
        return
    fi
...
complete -F _gold_completion gold
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for gold profile names."""
    return '''#compdef gold
# Gold Agent profile completion
# Add to ~/.zshrc: eval "$(gold completion zsh)"

_gold() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.gold/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.gold/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a GOLD_HOME path string.

    Called early in the CLI entry point, before any gold modules
    are imported, to set the GOLD_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: gold profile create {profile_name}"
        )
