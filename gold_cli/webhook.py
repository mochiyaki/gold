"""gold webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    gold webhook subscribe <name> [options]
    gold webhook list
    gold webhook remove <name>
    gold webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.gold/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _gold_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: gold gateway run
"""
...
def webhook_command(args):
    """Entry point for 'gold webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: gold webhook {subscribe|list|remove|test}")
        print("Run 'gold webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (gold gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: gold webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from gold webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (gold gateway run)")
