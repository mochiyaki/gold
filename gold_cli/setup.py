"""
Interactive setup wizard for Gold Agent.
...
Config files are stored in ~/.gold/ for easy access.
"""
...
_DOCS_BASE = "https://gold.mochiyaki.com/docs"
...
# display_gold_home imported lazily at call sites (stale-module safety during gold update)

from gold_cli.colors import Colors, color
...
from gold_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("◆ Gold Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Gold using environment variables or config commands:")
    print_info("  gold config set model.provider custom")
    print_info("  gold config set model.base_url http://localhost:8080/v1")
    print_info("  gold config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'gold setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0) -> int:
    ...
    from gold_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from gold_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'gold setup')")


def _print_setup_summary(config: dict, gold_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'gold setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'gold setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'gold setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'gold setup tools' to configure them,"
        )
        from gold_constants import display_gold_home as _dhh
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {gold_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('gold setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('gold setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('gold setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('gold setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('gold setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('gold config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('gold config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('gold config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('gold', Colors.GREEN)}              Start chatting")
    print(f"   {color('gold gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('gold doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `gold tools` and `gold setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``gold model``)
    ...
    provider added to ``gold model`` is automatically available here.
    ...
    # Delegate to the shared gold model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from gold_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: gold model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from gold_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Gold can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Gold can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'gold setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.gold/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'gold setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Gold runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Gold via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("GOLD_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `gold setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("GOLD_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("GOLD_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Gold delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://gold.mochiyaki.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Gold delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Gold delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'gold whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'gold whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from gold_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from gold_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from gold_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from gold_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from gold_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from gold_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from gold_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from gold_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_qqbot():
    ...
    print_info("Connects Gold to QQ via the Official QQ Bot API (v2).")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Gold to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from gold_cli.gateway import _PLATFORMS
    qq_platform = next((p for p in _PLATFORMS if p["key"] == "qqbot"), None)
    if qq_platform:
        from gold_cli.gateway import _setup_standard_platform
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://gold.mochiyaki.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://gold.mochiyaki.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  gold config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Gold from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'gold setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     gold config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: gold gateway install")
            else:
                print_info("  You can install later: gold gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo gold gateway install --system")
                print_info("  Or run in foreground:  gold gateway")
        else:
            from gold_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   gold gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   gold gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `gold setup tools` and `gold tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from gold_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from gold_cli.auth import get_active_provider
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_gold migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_gold", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Gold.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Gold to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Gold at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Gold at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Gold at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Gold at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Gold equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Gold config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Gold.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Gold's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(gold_home: Path) -> bool:
    ...
    print_info("Gold can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: gold claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: gold claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Gold configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Gold config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Gold (use gold claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      gold setup           — full or quick (auto-detected)
      gold setup model     — just model/provider
      gold setup tts       — just text-to-speech
      gold setup terminal  — just terminal backend
      gold setup gateway   — just messaging platforms
      gold setup tools     — just tool configuration
      gold setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ◆ Gold Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ◆ Gold Agent Setup Wizard                   │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Gold Agent installation.          │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Gold configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, gold_home)
            return
        elif choice == 1:
            # Full setup — fall through to run all sections
            pass
        elif choice == 7:
            print_info("Exiting. Run 'gold setup' again when ready.")
            return
        elif 2 <= choice <= 6:
        ...
        setup_mode = prompt_choice("How would you like to set up Gold?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, gold_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {gold_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'gold config edit'")
...
def _run_first_time_quick_setup(config: dict, gold_home, is_existing: bool):
...
def _run_quick_setup(config: dict, gold_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'gold setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Gold to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'gold setup gateway'.")
