"""
Status command for gold CLI.

Shows the status of all Gold Agent components.
"""
...
from gold_cli.auth import AuthError, resolve_provider
from gold_cli.colors import Colors, color
from gold_cli.config import get_env_path, get_env_value, get_gold_home, load_config
from gold_cli.models import provider_label
from gold_cli.nous_subscription import get_nous_subscription_features
from gold_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all Gold Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ◆ Gold Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from gold_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: gold model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: gold model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: gold model)"
    ...
    if not terminal_env:
        ...
        # (gold status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    if _is_termux():
        try:
            from gold_cli.gateway import find_gateway_pids
        ...
        from gold_cli.gateway import get_launchd_label
        try:
            result = subprocess.run(
    ...
    jobs_file = get_gold_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_gold_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'gold doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'gold setup' to configure", Colors.DIM))
    print()
