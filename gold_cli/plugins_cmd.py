"""``gold plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.gold/plugins/``.
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from gold_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_gold_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_gold_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_gold_home()}/.env later)[/dim]")
...
def cmd_install(identifier: str, force: bool = False) -> None:
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]gold plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    if not (target / "plugin.yaml").exists() and not (target / "__init__.py").exists():
        console.print(
            ...
            f"or __init__.py. It may not be a valid Gold plugin."
        )
    ...
    console.print("[dim]  gold gateway restart[/dim]")
    console.print()
...
def cmd_list() -> None:
    ...
    if not dirs:
        ...
        console.print("[dim]Install with:[/dim] gold plugins install owner/repo")
        return
    ...
    for d in dirs:
        ...
        # Check if it's a git repo (installed via gold plugins install)
        if (d / ".git").exists():
            source = "git"
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] gold plugins")
    console.print("[dim]Enable/disable:[/dim] gold plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch gold plugins subcommands."""
