"""
Gateway subcommand for gold CLI.

Handles: gold gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_gold_home is imported lazily at call sites to avoid ImportError
# when gold_constants is cached from a pre-update version during `gold update`.
from gold_cli.setup import (
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "gold-gateway*",
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``gold update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Gold profile are returned.
    """
    _exclude = exclude_pids or set()
    pids = [pid for pid in _get_service_pids() if pid not in _exclude]
    patterns = [
        "gold_cli.main gateway",
        "gold_cli.main --profile",
        "gold_cli.main -p",
        "gold_cli/main.py gateway",
        "gold_cli/main.py --profile",
        "gold_cli/main.py -p",
        "gold gateway",
        "gateway/run.py",
    ]
    current_home = str(get_gold_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"GOLD_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "GOLD_HOME=" in command and f"GOLD_HOME={current_home}" not in command:
            return False
        return True
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (GOLD_HOME-scoped).
...
from gold_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "gold-gateway"
SERVICE_DESCRIPTION = "Gold Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current GOLD_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (GOLD_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from gold_constants import get_default_gold_root
    home = get_gold_home().resolve()
    default = get_default_gold_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary GOLD_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(gold_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when GOLD_HOME is a named profile.

    For ``~/.gold/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        gold_home: Optional explicit GOLD_HOME path. Defaults to the current
            ``get_gold_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from gold_constants import get_default_gold_root
    home = Path(gold_home or str(get_gold_home())).resolve()
    default = get_default_gold_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this GOLD_HOME.

    Default ``~/.gold`` returns ``gold-gateway`` (backward compatible).
    Profile ``~/.gold/profiles/coder`` returns ``gold-gateway-coder``.
    Any other GOLD_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    gold gateway uninstall")
    print_info("    sudo gold gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Gold can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo gold gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo gold gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo gold gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Gold often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.gold`` → ``ai.gold.gateway.plist`` (backward compatible).
    Profile ``~/.gold/profiles/coder`` → ``ai.gold.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.gold.gateway-{suffix}" if suffix else "ai.gold.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.gold/gold  -> /home/alice/.gold/gold
      /opt/gold                 -> /opt/gold  (kept as-is)
...
def _gold_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current GOLD_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_gold_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.gold                    → /home/alice/.gold
      /root/.gold/profiles/coder     → /home/alice/.gold/profiles/coder
      /opt/custom-gold               → /opt/custom-gold  (kept as-is)
    """
    current_gold = get_gold_home().resolve()
    current_default = (Path.home() / ".gold").resolve()
    target_default = Path(target_home_dir) / ".gold"

    # Default ~/.gold → remap to target user's default
    if current_gold == current_default:
        return str(target_default)

    # Profile or subdir of ~/.gold → preserve the relative structure
    try:
        relative = current_gold.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.gold) — keep as-is
        return str(current_gold)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        gold_home = _gold_home_for_target_user(home_dir)
        profile_arg = _profile_arg(gold_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m gold_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="GOLD_HOME={gold_home}"
Restart=on-failure
RestartSec=30
...
"""
    ...
    gold_home = str(get_gold_home().resolve())
    profile_arg = _profile_arg(gold_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Gold install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}gold gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}gold gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        print(
            ...
            f"  Check status: {'sudo ' if system else ''}gold gateway status\n"
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}gold gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}gold gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}gold gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.gold.gateway-{suffix}" if suffix else "ai.gold.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Gold install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  gold gateway status             # Check status")
...
def launchd_stop():
    ...
    # `gold gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Gold install")
    else:
        print("⚠ Service definition is stale relative to the current Gold install")
        print("  Run: gold gateway start")
    ...
    if deep:
        log_file = get_gold_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Gold will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @gold:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. gold) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Gold delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Gold agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Gold will use (e.g., gold@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Gold connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: gold pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_gold_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'gold pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: gold pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'gold gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from gold_cli.main import cmd_whatsapp
    import argparse
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Gold will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Gold will store the returned account_id/token in ~/.gold/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `gold gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_gold_home())))
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `gold pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `gold pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"GoldAgent\"")
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: gold gateway")
                except subprocess.CalledProcessError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  gold gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: gold gateway run")
                print_info("  For persistence:   tmux new -s gold 'gold gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: gold gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup gold gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: gold gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'gold gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: gold gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: gold gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s gold 'gold gateway run'")
                print()
            ...
            print()
            print("  gold gateway run                              # direct foreground")
            print("  tmux new -s gold 'gold gateway run'         # persistent via tmux")
            print("  nohup gold gateway run > ~/.gold/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: gold gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: gold gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: gold gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: gold gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: gold gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  gold gateway install")
                    print("  sudo gold gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  gold gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup gold gateway run > ~/.gold/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s gold 'gold gateway run'         # persistent via tmux")
                    print("  nohup gold gateway run > ~/.gold/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  gold gateway install  # Install as user service")
                    print("  sudo gold gateway install --system  # Install as boot-time system service")
