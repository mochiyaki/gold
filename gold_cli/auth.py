"""
Multi-provider authentication system for Gold Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.gold/auth.json with cross-process file locking.
...
from gold_cli.config import get_gold_home, get_config_path, read_raw_config
...
DEFAULT_NOUS_PORTAL_URL = "https://portal.mochiyaki.com"
DEFAULT_NOUS_INFERENCE_URL = "https://inference-api.mochiyaki.com/v1"
DEFAULT_NOUS_CLIENT_ID = "gold-cli"
...
def get_anthropic_key() -> str:
    ...
    """
    from gold_cli.config import get_env_value
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from gold_cli.copilot_auth import resolve_copilot_token
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `gold model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("GOLD_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.gold/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    return get_gold_home() / "auth.json"
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Gold.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `gold logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from gold_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'gold doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'gold model' for available providers, or run 'gold doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'gold model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.gold/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("GOLD_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.gold/auth.json (not ~/.codex/)
#
# Gold maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Gold auth store (~/.gold/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `gold auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `gold auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `gold auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `gold auth` to re-authenticate.",
            provider="openai-codex",
...
def _write_codex_cli_tokens(
    ...
    *,
    ...
    When Gold refreshes a token it consumes the old refresh_token; if we
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Gold auth store (~/.gold/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Gold auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `gold auth` to re-authenticate."
            )
            relogin_required = True
        raise AuthError(
            message,
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Gold auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Gold's own Codex token store."""
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to Gold auth store")
            print("⚠️  Migrating Codex credentials to Gold's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `gold auth` to create a fully independent session.\n")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("GOLD_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Gold processes
    ...
    base_url = (
        os.getenv("GOLD_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    *,
    ...
    if effective_ca:
        ...
        if not os.path.isfile(ca_path):
            import logging
            logging.getLogger("gold.auth").warning(
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Gold models — they're not reliable for agentic tool-calling
            if "gold" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Gold is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("GOLD_PORTAL_BASE_URL")
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Gold is not logged into Nous Portal.",
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `gold status` output.

    Checks the credential pool first (where the dashboard device-code flow
    and ``gold auth`` store credentials), then falls back to the legacy
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `gold auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `gold auth` and
    # `gold model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("GOLD_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("GOLD_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set GOLD_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _prompt_model_selection(
    ...
    """
    from gold_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from gold_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'gold model' or 'gold setup' instead."""
    print("The 'gold login' command has been removed.")
    print("Use 'gold auth' to manage credentials,")
    print("'gold model' to select a provider, or 'gold setup' for full setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.gold/auth.json."""

    # Check for existing Gold-owned credentials
    try:
        ...
        if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
            print("Existing Codex credentials found in Gold auth store.")
            try:
    ...
    if cli_tokens:
        ...
        print("Gold will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            _save_codex_tokens(cli_tokens)
            base_url = os.getenv("GOLD_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
            config_path = _update_config_for_provider("openai-codex", base_url)
            print()
            print("Credentials imported. Note: if Codex CLI refreshes its token,")
            print("Gold will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — Gold gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Gold creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Gold auth store
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Gold login via {pconfig.name}...")
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `gold model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("Gold will use OpenRouter for inference.")
        else:
            print("Run `gold model` or configure an API key to use Gold.")
    else:
