...
from gold_cli.auth import get_nous_auth_status
from gold_cli.config import get_env_value, load_config
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        ...
        "Change these later with: gold setup tools, gold setup terminal, or gold status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `gold setup` and `gold model`."""
    if not managed_nous_tools_enabled():
        return set()
