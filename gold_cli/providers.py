"""
Single source of truth for provider identity in Gold Agent.
...
2. **Gold overlays** — transport type, auth patterns, aggregator flags,
...
# -- Gold overlay ----------------------------------------------------------
# Gold-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class GoldOverlay:
    """Gold-specific provider metadata layered on top of models.dev."""
...
GOLD_OVERLAYS: Dict[str, GoldOverlay] = {
    "openrouter": GoldOverlay(
    ...
    "nous": GoldOverlay(
        ...
        base_url_override="https://inference-api.mochiyaki.com/v1",
    ),
    "openai-codex": GoldOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": GoldOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="GOLD_QWEN_BASE_URL",
    ),
    "copilot-acp": GoldOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": GoldOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": GoldOverlay(
    ...
    "minimax-cn": GoldOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": GoldOverlay(
        transport="openai_chat",
    ...
    "opencode": GoldOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": GoldOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "gold", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Gold overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Gold overlay
    ...
    overlay = GOLD_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + gold extra
    ...
    if overlay is not None:
        # Gold-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
