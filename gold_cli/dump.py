"""
Dump command for gold CLI.

Outputs a compact, plain-text summary of the user's Gold setup
...
from gold_cli.config import get_gold_home, get_env_path, get_project_root, load_config
...
def _count_skills(gold_home: Path) -> int:
...
def _cron_summary(gold_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = gold_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"gold_home:      {display_gold_home()}")
    ...
    toolsets = config.get("toolsets", ["gold-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(gold_home)}")
    lines.append(f"  skills:             {_count_skills(gold_home)}")
