...
- ``gold_cli.providers``        -- canonical provider identity + overlays
- ``gold_cli.model_normalize``  -- per-provider name formatting
...
from gold_cli.providers import (
...
_GOLD_MODEL_WARNING = (
    "Nous Research Gold 3 & 4 models are NOT agentic and are not designed "
    "for use with Gold Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Gold 3 / Gold 4 chat families.
# The previous substring check (`"gold" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``gold-brain:qwen3-14b-ctx16k`` that just
# happen to carry "gold" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   mochiyaki/Gold-3-Llama-3.1-70B, gold-4-405b, openrouter/gold3:70b
# Negative examples it must NOT match:
#   gold-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_GOLD_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])gold[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_gold_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Gold 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_GOLD_NON_AGENTIC_RE.search(model_name))


def _check_gold_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Gold 3/4 chat model."""
    if is_nous_gold_non_agentic(model_name):
        return _GOLD_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    from gold_cli.runtime_provider import resolve_runtime_provider
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'gold model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'gold doctor' — config issues detected:"
    ...
    gold_warn = _check_gold_model_warning(new_model)
    if gold_warn:
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from gold_cli/models.py (OPENROUTER_MODELS,
    ...
    from gold_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by gold provider ID
    ...
    # --- 1. Check Gold-mapped providers ---
    for gold_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(gold_id)
    ...
    # --- 2. Check Gold-only providers (nous, openai-codex, copilot, opencode-go) ---
    from gold_cli.providers import GOLD_OVERLAYS
    from gold_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Gold provider ID.
    # GOLD_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Gold IDs ("copilot").
    _mdev_to_gold = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in GOLD_OVERLAYS.items():
        if pid in seen_slugs:
            continue

        # Resolve Gold slug — e.g. "github-copilot" → "copilot"
        gold_slug = _mdev_to_gold.get(pid, pid)
        if gold_slug in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                from gold_cli.auth import _load_auth_store
                store = _load_auth_store()
                providers_store = store.get("providers", {})
                pool_store = store.get("credential_pool", {})
                if store and (
                    pid in providers_store or gold_slug in providers_store
                    or pid in pool_store or gold_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(gold_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", gold_slug, exc)
        ...
        # configured.
        if not has_creds and gold_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    read_claude_code_credentials,
                    read_gold_oauth_credentials,
                )
                gold_creds = read_gold_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (gold_creds and gold_creds.get("accessToken")) or \
        ...
        # Use curated list — look up by Gold slug, fall back to overlay key
        model_ids = curated.get(gold_slug, []) or curated.get(pid, [])
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": gold_slug,
            "name": get_label(gold_slug),
            "is_current": gold_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "gold",
        ...
        seen_slugs.add(pid)
        seen_slugs.add(gold_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or GOLD_OVERLAYS (keeps /model in sync
    # with `gold model`).
    try:
        from gold_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
