...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_gold_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:gold_pkce",
    ...
    raise SystemExit(f"`gold auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    if removed.source.startswith("env:"):
        ...
        if env_var:
            from gold_cli.config import remove_env_value
            cleared = remove_env_value(env_var)
            if cleared:
                print(f"Cleared {env_var} from .env")

    # If this was a singleton-seeded credential (OAuth device_code, gold_pkce),
    ...
    # re-seeded on the next load_pool() call.
    ...
    elif removed.source == "gold_pkce" and provider == "anthropic":
        from gold_constants import get_gold_home
        oauth_file = get_gold_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared Gold Anthropic OAuth credentials")

    elif removed.source == "claude_code" and provider == "anthropic":
        from gold_cli.auth import suppress_credential_source
        ...
        print("Run `gold auth add anthropic` to re-enable if needed.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `gold auth` is called bare."""
