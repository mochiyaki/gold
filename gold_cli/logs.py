"""``gold logs`` — view and filter Gold log files.
...
Usage examples::
    ...
    gold logs                    # last 50 lines of agent.log
    gold logs -f                 # follow agent.log in real time
    gold logs errors             # last 50 lines of errors.log
    gold logs gateway -n 100    # last 100 lines of gateway.log
    gold logs --level WARNING    # only WARNING+ lines
    gold logs --session abc123   # filter by session ID substring
    gold logs --component tools  # only tool-related lines
    gold logs --since 1h         # lines from the last hour
    gold logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_gold_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Gold runs — try 'gold chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_gold_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_gold_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_gold_home()}/logs/")
        return

    print(f"Log files in {display_gold_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'gold chat' to generate logs)")
