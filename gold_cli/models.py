...
Add, remove, or reorder entries here — both `gold setup` and
`gold` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `gold model`
    flow without maintaining a separate static list.
    """
    from gold_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def fetch_nous_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    ...
    """
    base = (portal_base_url or "https://portal.mochiyaki.com").rstrip("/")
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from gold_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  gold model, /model, /provider, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `gold model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `gold model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``gold model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``gold auth add openai-codex`` without
    ``gold model``).
    """
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.mochiyaki.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``gold model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from gold_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Gold' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``gold_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Gold should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from gold_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from gold_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from gold_cli.copilot_auth import copilot_request_headers
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from gold_constants import AI_GATEWAY_BASE_URL
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Gold will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
