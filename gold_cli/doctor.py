"""
Doctor command for gold CLI.

Diagnoses issues with Gold Agent setup.
"""
...
from gold_cli.config import get_project_root, get_gold_home, get_env_path
from gold_constants import display_gold_home

PROJECT_ROOT = get_project_root()
GOLD_HOME = get_gold_home()
_DHH = display_gold_home()  # user-facing display path (e.g. ~/.gold or ~/.gold/profiles/coder)

# Load environment variables from ~/.gold/.env so API key checks work
...
from gold_constants import OPENROUTER_MODELS_URL
...
from gold_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.gold/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `gold`.
    os.environ.setdefault("GOLD_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 Gold Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.gold/.env (primary location for user config)
    env_path = GOLD_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'gold setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'gold setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'gold setup' to create one")
                issues.append("Run 'gold setup' to create .env")
    
    # Check ~/.gold/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = GOLD_HOME / 'config.yaml'
    ...
    if config_path.exists():
        try:
            from gold_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'gold setup' to migrate config")
                else:
                    issues.append("Run 'gold doctor --fix' or 'gold setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'gold doctor --fix'")
        except Exception:
            pass
    ...
    try:
        from gold_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    if gold_home.exists():
        ...
        if should_fix:
            gold_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = gold_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Gold a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Gold Agent Persona\n\n"
                "<!-- Edit this file to customize how Gold communicates. -->\n\n"
                "You are Gold, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = gold_home / "memories"
    ...
    state_db_path = gold_home / "state.db"
    ...
    wal_path = gold_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'gold doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (gold bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "gold"
        ...
        _cmd_link = _cmd_link_dir / "gold"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(gold not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/gold → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/gold points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/gold → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/gold — run 'gold doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/gold exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/gold not found",
                    "(gold command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/gold → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/gold symlink — run 'gold doctor --fix'")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'gold setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: gold skills list)")
    ...
    try:
        ...
        _mem_cfg_path = GOLD_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: gold memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: gold memory setup")
                issues.append("No Honcho API key — run 'gold memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run gold memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: gold memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: gold memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from gold_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "gold -p" in content:
                            _m = _re.search(r"gold -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'gold doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
