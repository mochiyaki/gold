#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the Gold Skills Hub.

Powers both:
  - `gold skills <subcommand>` (CLI argparse entry point)
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    c.print(table)
    c.print("[dim]Use: gold skills inspect <identifier> to preview, "
            "gold skills install <identifier> to install[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    c.print("[dim]Tip: 'gold skills search <query>' searches deeper across all registries[/]\n")
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                "It ships with gold but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_gold_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="gold skills install <id> to install"))
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: gold skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: gold skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: gold skills publish <path> --to github --repo owner/repo\n")
            return

        auth = GitHubAuth()
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_gold_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Gold Skills Hub.\n\n"
                        f"This skill was scanned by the Gold Skills Guard before submission.",
...
def skills_command(args) -> None:
    """Router for `gold skills <subcommand>` — called from gold_cli/main.py."""
    ...
    if action == "browse":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: gold skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: gold skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: gold skills [browse|search|install|inspect|list|check|update|audit|uninstall|publish|snapshot|tap]\n")
        _console.print("Run 'gold skills <command> --help' for details.\n")
