...
Usage:
    gold pairing list              # Show all pending + approved users
    gold pairing approve <platform> <code>  # Approve a pairing code
    gold pairing revoke <platform> <user_id> # Revoke user access
    gold pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle gold pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: gold pairing {list|approve|revoke|clear-pending}")
        print("Run 'gold pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print("  Run 'gold pairing list' to see pending codes.\n")
