...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or gold -c.",
    ...
    "Ctrl+Z suspends Gold to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "gold -c resumes your most recent CLI session. gold -c \"project name\" resumes by title.",
    "gold -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "gold -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "gold chat -t web,terminal enables only specific toolsets for a focused session.",
    "gold chat -s github-pr-workflow preloads a skill at launch.",
    "gold chat -q \"query\" runs a single non-interactive query and exits.",
    "gold chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "gold chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "gold --yolo bypasses all dangerous command approval prompts for the entire session.",
    "gold chat --source telegram tags the session for filtering in gold sessions list.",
    "gold -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "gold doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "gold dump outputs a compact setup summary — great for bug reports.",
    "gold config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "gold config edit opens config.yaml in your default editor.",
    "gold config check scans for missing or stale configuration options.",
    "gold sessions browse opens an interactive session picker with search.",
    "gold sessions stats shows session counts by platform and database size.",
    "gold sessions prune --older-than 30 cleans up old sessions.",
    "gold skills search react --source skills-sh searches the skills.sh public directory.",
    "gold skills check scans installed hub skills for upstream updates.",
    "gold skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "gold skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "gold mcp add github --command npx adds MCP servers from the command line.",
    "gold mcp serve runs Gold itself as an MCP server for other agents.",
    "gold auth add lets you add multiple API keys for credential pool rotation.",
    "gold completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "gold logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "gold backup creates a zip backup of your entire Gold home directory.",
    "gold profile create coder creates an isolated profile that becomes its own command.",
    "gold profile create work --clone copies your current config and keys to a new profile.",
    "gold update syncs new bundled skills to ALL profiles automatically.",
    "gold gateway install sets up Gold as a system service (systemd/launchd).",
    "gold memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "gold webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as gold -w).",
    ...
    "GOLD_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Gold tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'gold profile create coder' creates the 'coder' command.",
    "gold profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Gold prints a resume command with session ID and stats.",
    "gold sessions export backup.jsonl exports all sessions for backup or analysis.",
    "gold -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "gold skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .gold/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: gold cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Gold runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "gold gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Gold via a local macOS server.",
    ...
    "Context files (.gold.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.gold/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Gold loads project context from .gold.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — gold tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.gold/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "gold chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "gold plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "gold acp runs Gold as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "GOLD_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "gold login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "gold status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.gold/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.gold/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Gold your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.gold/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Gold auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.gold/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at GOLD_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "GOLD_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in GOLD_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.gold/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'gold profile create ls' would be rejected.",
    "gold profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "gold claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "GOLD_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via gold tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.gold/.env.",
    "Skills from trusted repos (mochiyaki) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.gold/skills/.hub/quarantine/ holds skills pending security review.",
]
