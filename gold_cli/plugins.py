...
1. **User plugins**   – ``~/.gold/plugins/<name>/``
2. **Project plugins** – ``./.gold/plugins/<name>/`` (opt-in via
   ``GOLD_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``gold_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "gold_agent.plugins"

_NS_PARENT = "gold_plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``gold honcho ...``).
    ...
    def register_skill(
        ...
        ``~/.gold/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.gold/plugins/)
        user_dir = get_gold_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.gold/plugins/)
        if _env_enabled("GOLD_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".gold" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``gold_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``gold tools`` TUI so plugin-provided toolsets appear
