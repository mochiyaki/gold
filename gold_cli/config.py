"""
Configuration management for Gold Agent.

Config files are stored in ~/.gold/ for easy access:
- ~/.gold/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.gold/.env         - API keys and secrets

This module provides:
- gold config          - Show current configuration
- gold config edit     - Open config in editor
- gold config set      - Set a specific value
- gold config wizard   - Re-run setup wizard
"""
...
from gold_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("GOLD_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_gold_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Gold is running in package-manager-managed mode.

    Two signals: the GOLD_MANAGED env var (set by the systemd service),
    or a .managed marker file in GOLD_HOME (set by the NixOS activation
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "gold update"


def format_managed_message(action: str = "modify this Gold installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("GOLD_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Gold installation is managed by NixOS "
            f"(GOLD_MANAGED={env_hint}).\n"
            "Edit services.gold.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Gold installation is managed by Homebrew "
            f"(GOLD_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade gold"
        )
    ...
    return (
        f"Cannot {action}: this Gold installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Gold."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from GOLD_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, gold_bin
    or None if container mode is not active, we're already inside the
    container, or GOLD_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("GOLD_DEV") == "1":
        return None
    ...
    container_mode_file = get_gold_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "gold")
    exec_user = info.get("exec_user", "gold")
    gold_bin = info.get("gold_bin", "/data/current-package/bin/gold")
...
# Re-export from gold_constants — canonical definition lives there.
from gold_constants import get_gold_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_gold_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_gold_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the gold group can
    share state with the gateway service.

    The mode can be overridden via the GOLD_HOME_MODE environment variable
    (e.g. GOLD_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse GOLD_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("GOLD_HOME_MODE", "").strip()
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into GOLD_HOME if the user doesn't have one yet."""
...
def ensure_gold_home():
    """Ensure ~/.gold directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_gold_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_gold_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"GOLD_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when Gold
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Gold sends a stable profile-scoped userId to Camofox
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.gold/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.gold/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for gold-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "◆ *Gold Agent*" header.
    ...
    # Logging — controls file logging to ~/.gold/logs/.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. mochiyaki.com -> firecrawl-gateway.mochiyaki.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Gold auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @gold:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. GOLD_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'gold' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Gold API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Gold API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Gold API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # GOLD_TOOL_PROGRESS and GOLD_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.gold.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'gold setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Gold won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'gold doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("GOLD_TOOL_PROGRESS")
            old_mode = get_env_value("GOLD_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from GOLD_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from GOLD_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("GOLD_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from GOLD_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: gold config set <key> <value>")
    ...
    # metadata.gold.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.gold/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.gold/config.yaml."""
    import copy
    ensure_gold_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — gold auth) — OpenAI Codex
#   nous         (OAuth — gold auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.gold/config.yaml."""
    if is_managed():
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Gold env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.gold/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.gold/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.gold/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.gold/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Gold — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Gold vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.gold/.env or environment."""
...
def show_config():
    ...
    print(color("│              ◆ Gold Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  gold config edit     # Edit config file", Colors.DIM))
    print(color("  gold config set <key> <value>", Colors.DIM))
    print(color("  gold setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: gold config set <key> <value>")
            print()
            print("Examples:")
            print("  gold config set model anthropic/claude-sonnet-4")
            print("  gold config set terminal.backend docker")
            print("  gold config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'gold config migrate' to add them")
        ...
        print("  gold config           Show current configuration")
        print("  gold config edit      Open config in editor")
        print("  gold config set <key> <value>   Set a config value")
        print("  gold config check     Check for missing/outdated config")
        print("  gold config migrate   Update config with new options")
        print("  gold config path      Show config file path")
        print("  gold config env-path  Show .env file path")
        sys.exit(1)
