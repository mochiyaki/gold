"""Shell completion script generation for gold CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_gold_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Gold Agent bash completion
# Add to ~/.bashrc:
#   eval "$(gold completion bash)"
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef gold
# Gold Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(gold completion zsh)"
...
_gold() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_gold_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'gold command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Gold Agent fish completion",
        "# Add to your config:",
        "#   gold completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __gold_profiles",
        "    echo default",
        "    if test -d $HOME/.gold/profiles",
        "        ls $HOME/.gold/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c gold -f -s p -l profile"
        " -d 'Profile name' -xa '(__gold_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__gold_profiles)' -d 'Profile name'"
                )
