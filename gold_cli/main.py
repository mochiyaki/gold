#!/usr/bin/env python3
"""
Gold CLI - Main entry point.

Usage:
    gold                     # Interactive chat (default)
    gold chat                # Interactive chat
    gold gateway             # Run gateway in foreground
    gold gateway start       # Start gateway as service
    gold gateway stop        # Stop gateway service
    gold gateway status      # Show gateway status
    gold gateway install     # Install gateway service
    gold gateway uninstall   # Uninstall gateway service
    gold setup               # Interactive setup wizard
    gold logout              # Clear stored authentication
    gold status              # Show status of all components
    gold cron                # Manage cron jobs
    gold cron list           # List cron jobs
    gold cron status         # Check if cron scheduler is running
    gold doctor              # Check configuration and dependencies
    gold honcho setup                    # Configure Honcho AI memory integration
    gold honcho status                   # Show Honcho config and connection status
    gold honcho sessions                 # List directory → session name mappings
    gold honcho map <name>               # Map current directory to a session name
    gold honcho peer                     # Show peer names and dialectic settings
    gold honcho peer --user NAME         # Set user peer name
    gold honcho peer --ai NAME           # Set AI peer name
    gold honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    gold honcho mode                     # Show current memory mode
    gold honcho mode [hybrid|honcho|local]  # Set memory mode
    gold honcho tokens                   # Show token budget settings
    gold honcho tokens --context N       # Set session.context() token cap
    gold honcho tokens --dialectic N     # Set dialectic result char cap
    gold honcho identity                 # Show AI peer identity representation
    gold honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    gold honcho migrate                  # Step-by-step migration guide: OpenClaw native → Gold + Honcho
    gold version             Show version
    gold update              Update to latest version
    gold uninstall           Uninstall Gold Agent
    gold acp                 Run as an ACP server for editor integration
    gold sessions browse     Interactive session picker with search

    gold claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (gold tools, gold setup, gold model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'gold {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any gold module import.
#
# Many modules cache GOLD_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("GOLD_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.gold/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set GOLD_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check active_profile in the gold root
    if profile_name is None:
        try:
            from gold_constants import get_default_gold_root
            active_path = get_default_gold_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set GOLD_HOME
    if profile_name is not None:
        try:
            from gold_cli.profiles import resolve_profile_env
            gold_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent gold from starting
...
# Load .env from ~/.gold/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from gold_cli.config import get_gold_home
from gold_cli.env_loader import load_gold_dotenv
load_gold_dotenv(project_env=PROJECT_ROOT / '.env')

# Initialize centralized file logging early — all `gold` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from gold_logging import setup_logging as _setup_logging
...
try:
    from gold_cli.config import load_config as _load_config_early
    from gold_constants import apply_ipv4_preference as _apply_ipv4
...
from gold_cli import __version__, __release_date__
from gold_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from gold_cli.config import get_env_path, get_gold_home, load_config
    from gold_cli.auth import get_auth_status

    # Determine whether Gold itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from gold_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_gold_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from gold_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_gold_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Gold has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Gold to use their tokens.
    if _has_gold_config:
        try:
...
def _resolve_last_cli_session() -> Optional[str]:
    ...
    try:
        from gold_state import SessionDB
        db = SessionDB()
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, gold_bin
        cli_args: the original CLI arguments (everything after 'gold')
    """
    import shutil
    ...
    gold_bin = container_info["gold_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f'  }}];\n'
                    f"\n"
                    f"Or run: sudo gold {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo gold {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'gold sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Gold isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  gold setup")
        print()

        from gold_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'gold setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from gold_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["GOLD_YOLO_MODE"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["GOLD_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from gold_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from gold_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Gold?")
        print()
    ...
    session_dir = get_gold_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: gold gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  gold gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '◆ Gold Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  gold gateway")
        ...
        print()
        print("  Or install as a service: gold gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'gold whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from gold_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``gold model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from gold_cli.auth import (
        resolve_provider, AuthError, format_auth_error,
    )
    from gold_cli.config import get_compatible_custom_providers, load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("GOLD_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
    ...
    from gold_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.gold/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.gold/.env if the active provider is not 'custom'.
    ...
    """
    from gold_cli.config import get_env_value, save_env_value, load_config
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from gold_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from gold_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from gold_cli.config import get_env_value, save_env_value
    ...
    from gold_cli.models import model_ids, get_pricing_for_provider
    ...
    if selected:
        ...
        from gold_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from gold_cli.config import get_env_value, save_config, save_env_value
    from gold_cli.nous_subscription import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from gold_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from gold_cli.codex_models import get_codex_model_ids
    import argparse
    ...
    _codex_token = None
    # Prefer credential pool (where `gold auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from gold_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from gold_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from gold_cli.auth import _save_model_choice, deactivate_provider
    from gold_cli.config import get_env_value, load_config, save_config
    ...
    from gold_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `gold model` to set a model.")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from gold_cli.curses_ui import flush_stdin
        flush_stdin()
        print()
...
from gold_cli.models import _PROVIDER_MODELS
...
def _model_flow_copilot(config, current_model=""):
    ...
    from gold_cli.config import save_env_value, load_config, save_config
    from gold_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from gold_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from gold_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Gold turns to `copilot --acp`.")
    print("  Gold currently starts its own ACP subprocess for each request.")
    print("  Gold uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set GOLD_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from gold_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from gold_cli.config import get_env_value, save_env_value, load_config, save_config
    from gold_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from gold_constants import display_gold_home as _dhh_fn
            print(f"    Gold will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from gold_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'gold model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Gold CLI with a provider."""
    from gold_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from gold_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from gold_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from gold_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from gold_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from gold_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from gold_cli.doctor import run_doctor
    run_doctor(args)


def cmd_dump(args):
    """Dump setup summary for support/debugging."""
    from gold_cli.dump import run_dump
    run_dump(args)


def cmd_debug(args):
    """Debug tools (share report, etc.)."""
    from gold_cli.debug import run_debug
    run_debug(args)


def cmd_config(args):
    """Configuration management."""
    from gold_cli.config import config_command
    config_command(args)


def cmd_backup(args):
    """Back up Gold home directory to a zip file."""
    if getattr(args, "quick", False):
        from gold_cli.backup import run_quick_backup
        run_quick_backup(args)
    else:
        from gold_cli.backup import run_backup
        run_backup(args)


def cmd_import(args):
    """Restore a Gold backup from a zip file."""
    from gold_cli.backup import run_import
    run_import(args)


def cmd_version(args):
    """Show version."""
    print(f"Gold Agent v{__version__} ({__release_date__})")
    ...
    try:
        from gold_cli.banner import check_for_updates
        from gold_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Gold Agent."""
    _require_tty("uninstall")
    from gold_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``gold update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``gold web``).
    ...
    if r1.returncode != 0:
        print(f"  {'✗' if fatal else '⚠'} Web UI npm install failed"
              + ("" if fatal else " (gold web will not be available)"))
        if fatal:
...
def _update_via_zip(args):
    """Update Gold Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/mochiyaki/gold/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="gold-update-")
        zip_path = os.path.join(tmp_dir, f"gold-{branch}.zip")
        ...
        # GitHub ZIPs extract to gold-<branch>/
        extracted = os.path.join(tmp_dir, f"gold-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("gold-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Gold behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes gold completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Gold couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Gold couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Gold behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `gold update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/mochiyaki/gold.git",
    "git@github.com:mochiyaki/gold.git",
    "https://github.com/mochiyaki/gold",
    "git@github.com:mochiyaki/gold",
}
OFFICIAL_REPO_URL = "https://github.com/mochiyaki/gold.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    from gold_constants import get_gold_home
    return (get_gold_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from gold_constants import get_gold_home
        (get_gold_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Gold repository.")
        print("  This means you may miss updates from mochiyaki/gold.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/mochiyaki/gold.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/mochiyaki/gold.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``gold update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from gold_constants import get_default_gold_root
    default_home = get_default_gold_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "gold[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update Gold Agent to the latest version."""
    import shutil
    from gold_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Gold Agent")
        return
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/mochiyaki/gold/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_gold_home added to gold_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload gold_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_gold_home() added since the last release.
        try:
            import importlib
            import gold_constants as _hc
        ...
        try:
            from gold_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'gold config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: gold config migrate")
            else:
                print()
                print("Skipped. Run 'gold config migrate' later to configure.")
        else:
        ...
        # When running as ``gold update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart gold-gateway`` kills everything in the
        ...
        if gateway_mode:
            _exit_code_path = get_gold_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all gold-gateway* units (default + profiles)
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "gold-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. gold-gateway.service or gold-gateway-coder.service
            ...
            if is_macos():
                try:
                    from gold_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: gold gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: gold -p <profile> gateway run  for each profile)")
        ...
        print("  gold model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``gold -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `gold profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_gold_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → gold -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.gold)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  gold profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     gold -p {name} chat")
                else:
        ...
        name = args.profile_name
        from gold_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from gold_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from gold_cli.profiles import rename_profile
        try:
        ...
        from gold_cli.profiles import export_profile
        name = args.profile_name
        ...
        from gold_cli.profiles import import_profile
        try:
...
def cmd_dashboard(args):
    ...
    try:
        ...
        print("Install them with:  pip install gold[web]")
        sys.exit(1)

    if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
        sys.exit(1)

    from gold_cli.web_server import start_server
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from gold_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Gold log files."""
    from gold_cli.logs import tail_log, list_logs
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="gold",
        description="Gold Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    gold                        Start interactive chat
    gold chat -q "Hello"        Single query mode
    gold -c                     Resume the most recent session
    gold -c "my project"        Resume a session by name (latest in lineage)
    gold --resume <session_id>  Resume a specific session by ID
    gold setup                  Run setup wizard
    gold logout                 Clear stored authentication
    gold auth add <provider>    Add a pooled credential
    gold auth list              List pooled credentials
    gold auth remove <p> <t>    Remove pooled credential by index, id, or label
    gold auth reset <provider>  Clear exhaustion status for a provider
    gold model                  Select default model
    gold config                 View configuration
    gold config edit            Edit config in $EDITOR
    gold config set model gpt-4 Set a config value
    gold gateway                Run messaging gateway
    gold -s gold-dev,github-auth
    gold -w                     Start in isolated git worktree
    gold gateway install        Install gateway background service
    gold sessions list          List past sessions
    gold sessions browse        Interactive session picker
    gold sessions rename ID T   Rename/title a session
    gold logs                   View agent.log (last 50 lines)
    gold logs -f                Follow agent.log in real time
    gold logs errors            View errors.log
    gold logs --since 1h        Lines from the last hour
    gold debug share             Upload debug report for support
    gold update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Gold Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: gold-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Gold Agent with an interactive wizard. "
                    "Run a specific section: gold setup model|tts|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Gold CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: gold-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Gold Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Gold Agent setup"
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Gold setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Gold Agent. Use 'gold debug share' to "
                    "upload a debug report (system info + recent logs) to a paste "
                    "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    gold debug share              Upload debug report and print URL
    gold debug share --lines 500  Include more log lines
    gold debug share --expire 30  Keep paste for 30 days
    gold debug share --local      Print report locally (no upload)
""",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Gold home directory to a zip file",
        description="Create a zip archive of your entire Gold configuration, "
                    "skills, sessions, and data (excludes the gold codebase). "
                    "Use --quick for a fast snapshot of just critical state files."
    )
    backup_parser.add_argument(
        "-o", "--output",
        help="Output path for the zip file (default: ~/gold-backup-<timestamp>.zip)"
    )
    backup_parser.add_argument(
        "-q", "--quick",
        action="store_true",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Gold backup from a zip file",
        description="Extract a previously created Gold backup into your "
                    "Gold home directory, restoring configuration, skills, "
                    "sessions, and data"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Gold Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from gold_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from gold_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from gold_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/gold-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from gold_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from gold_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'gold tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # gold tools disable <name...> [--platform cli]
    ...
    # gold tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from gold_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from gold_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Gold as an MCP server",
        description=(
            "Manage MCP server connections and run Gold as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'gold mcp add' to connect to a new server, or\n"
            "'gold mcp serve' to expose Gold conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Gold as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from gold_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch gold --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            gold_bin = shutil.which("gold")
            if gold_bin:
                os.execvp(gold_bin, ["gold", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "gold_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Gold"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Gold",
    ...
    def cmd_claw(args):
        from gold_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Gold Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Gold Agent",
        description="Remove Gold Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Gold Agent as an ACP (Agent Client Protocol) server",
        description="Start Gold Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Gold Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Gold instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Gold Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Gold log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    gold logs                    Show last 50 lines of agent.log
    gold logs -f                 Follow agent.log in real time
    gold logs errors             Show last 50 lines of errors.log
    gold logs gateway -n 100     Show last 100 lines of gateway.log
    gold logs --level WARNING    Only show WARNING and above
    gold logs --session abc123   Filter by session ID
    gold logs --component tools  Only show tool-related lines
    gold logs --since 1h         Lines from the last hour
    gold logs --since 30m -f     Follow, starting from 30 min ago
    gold logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``gold -c Pokemon Agent Dev`` → ``gold -c 'Pokemon Agent Dev'``
    ...
    from gold_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'gold -c model' where 'model' is consumed as the
