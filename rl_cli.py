...
if tinker_atropos_dir.exists():
    ...
    os.environ['GOLD_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to gold directory if submodule not found
...
from gold_constants import get_gold_home, OPENROUTER_BASE_URL
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.gold/config.yaml if not provided)
    ...
    """
    # Load config from ~/.gold/config.yaml
