...
Handles: gold honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "gold" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Gold profiles and creates host blocks for any that don't
    have one yet. Inherits settings from the default host block.
    """
    try:
        from gold_cli.profiles import list_profiles
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'gold honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'gold honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `gold update` -- no output, no exceptions.
    """
    try:
        from gold_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Gold profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $GOLD_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_gold_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Gold persistent cross-session memory.")
    ...
    gold_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'gold honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = gold_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        gold_host["peerName"] = new_peer

    current_ai = gold_host.get("aiPeer") or cfg.get("aiPeer", "gold")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        gold_host["aiPeer"] = new_ai

    current_workspace = gold_host.get("workspace") or cfg.get("workspace", "gold")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        gold_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = gold_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in ("unified", "directional"):
        gold_host["observationMode"] = new_obs
    else:
        gold_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(gold_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        gold_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        gold_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # --- 6. Recall mode ---
    _raw_recall = gold_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        gold_host["recallMode"] = new_recall

    # --- 7. Session strategy ---
    current_strat = gold_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-directory")
    print("\n  Session strategy:")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        gold_host["sessionStrategy"] = new_strat

    gold_host["enabled"] = True
    gold_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        gold_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(gold_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: gold config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    gold honcho status     -- show full config")
    print("    gold honcho mode       -- change recall/observation mode")
    print("    gold honcho tokens     -- tune context and dialectic budgets")
    print("    gold honcho peer       -- update peer names")
    print("    gold honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Gold profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: gold honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'gold honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: gold honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        gold = hosts.get(_host_key(), {})
        user = gold.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = gold.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = gold.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = gold.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Gold' identity in Honcho. Seed with 'gold honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: gold honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = gold.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = gold.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = gold.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Gold asks Honcho's AI peer a question")
        ...
        print("\n  Set with: gold honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'gold honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'gold honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    gold honcho identity --show        — show both peer representations")
        print("    gold honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Gold + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Gold\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Gold persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  gold honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'gold honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'gold honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  gold honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'gold'")
        print("  with Honcho configured and no prior session history.")
        print("  (Gold calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    gold honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'gold honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Gold, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'gold honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    gold honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  gold honcho identity <path/to/SOUL.md>")
    ...
    print("    Gold:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Gold:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Gold:   Honcho observes every message and updates representations")
    ...
    print("    Gold:   per-session by default — each run gets its own session")
    print("              Map a custom name:  gold honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. gold honcho setup              — configure API key (required)")
        print("  2. gold honcho migrate            — re-run this walkthrough")
    else:
        print("  1. gold honcho status             — verify Honcho connection")
        print("  2. gold                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. gold honcho identity --show    — verify AI peer representation")
        print("  4. gold honcho tokens             — tune context and dialectic budgets")
        print("  5. gold honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'gold memory setup'...\n")
        from gold_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``gold honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``gold honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to gold memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Gold Honcho",
    )
