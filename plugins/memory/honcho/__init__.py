...
Config: Uses the existing Honcho config chain:
  1. $GOLD_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, gold_home):
        """Write config to $GOLD_HOME/honcho.json (Honcho SDK native format)."""
        import json
        from pathlib import Path
        config_path = Path(gold_home) / "honcho.json"
        existing = {}
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        try:
            if not session.messages:
                ...
                mem_dir = str(get_gold_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "gold-default",
