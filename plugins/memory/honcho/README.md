...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/gold>
...
```bash
gold honcho setup    # full interactive wizard (cloud or local)
gold memory setup    # generic picker, also works
```

Or manually:
```bash
gold config set memory.provider honcho
echo "HONCHO_API_KEY=your-key" >> ~/.gold/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$GOLD_HOME/honcho.json` | Profile-local (isolated Gold instances) |
| 2 | `~/.gold/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Gold profile: `gold` (default) or `gold.<profile>`.
...
Host key derivation: `GOLD_HONCHO_HOST` env > active profile (`gold.<profile>`) > `"gold"`.
...
| `GOLD_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `gold honcho setup` | Full interactive setup wizard |
| `gold honcho status` | Show resolved config for active profile |
| `gold honcho enable` / `disable` | Toggle Honcho for active profile |
| `gold honcho mode <mode>` | Change recall or observation mode |
| `gold honcho peer --user <name>` | Update user peer name |
| `gold honcho peer --ai <name>` | Update AI peer name |
| `gold honcho tokens --context <N>` | Set context token budget |
| `gold honcho tokens --dialectic <N>` | Set dialectic max chars |
| `gold honcho map <name>` | Map current directory to a session name |
| `gold honcho sync` | Create host blocks for all Gold profiles |
