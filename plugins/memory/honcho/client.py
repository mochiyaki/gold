...
Resolution order for config file:
  1. $GOLD_HOME/honcho.json  (instance-local, enables isolated Gold instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Gold profile.

    Resolution order:
      1. GOLD_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``gold.<profile>``
      3. Fallback: ``"gold"`` (default profile)
    """
    explicit = os.environ.get("GOLD_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $GOLD_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.gold/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_gold_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = Path.home() / ".gold" / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Gold system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.gold
    ...
    def from_global_config(
        ...
        Resolution: $GOLD_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Gold profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.gold block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Gold session title (from /title command)
          3. per-session strategy — Gold session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Gold session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'gold honcho setup' or set HONCHO_API_KEY. "
    ...
    if not resolved_base_url:
        try:
            ...
            honcho_cfg = gold_cfg.get("honcho", {})
