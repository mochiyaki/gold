...
```bash
gold memory setup    # select "hindsight"
```
...
```bash
gold config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.gold/.env
```
...
Gold spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.gold/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p gold ui start
```
...
Config file: `~/.gold/hindsight/config.json`
...
| `retain_context` | `conversation between Gold Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.gold/.env` as `HINDSIGHT_LLM_API_KEY`.
