...
```bash
gold memory setup    # select "supermemory"
```

Or manually:

```bash
gold config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.gold/.env
```

## Config

Config file: `$GOLD_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `gold` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `gold-{identity}` → `gold-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Gold profile:

```json
{
  "container_tag": "gold-{identity}"
}
```

For a profile named `coder`, this resolves to `gold-coder`. The default profile resolves to `gold-default`. Without `{identity}`, all profiles share the same container.
