...
def _load_supermemory_config(gold_home: str) -> dict:
    config = _default_config()
    config_path = Path(gold_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, gold_home: str) -> None:
    config_path = Path(gold_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `gold memory setup`.
        # All other options are documented for $GOLD_HOME/supermemory.json
    ...
    def save_config(self, values, gold_home):
        ...
        _save_supermemory_config(sanitized, gold_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from gold_constants import get_gold_home
        self._gold_home = kwargs.get("gold_home") or str(get_gold_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._gold_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "gold", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "gold_memory", "target": target, "type": "explicit_memory"},
