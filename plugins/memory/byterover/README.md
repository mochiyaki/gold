...
```bash
gold memory setup    # select "byterover"
```

Or manually:
```bash
gold config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.gold/.env
```
...
Working directory: `$GOLD_HOME/byterover/` (profile-scoped).
