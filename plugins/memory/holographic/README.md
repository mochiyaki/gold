...
```bash
gold memory setup    # select "holographic"
```

Or manually:
```bash
gold config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.gold-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$GOLD_HOME/memory_store.db` | SQLite database path |
