"""
Gold MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Gold-specific extra)

Usage:
    gold mcp serve
    gold mcp serve --verbose
...
logger = logging.getLogger("gold.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using GOLD_HOME."""
    try:
        from gold_constants import get_gold_home
        return get_gold_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("GOLD_HOME", Path.home() / ".gold")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from gold_constants import get_gold_home
        directory_file = get_gold_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("GOLD_HOME", Path.home() / ".gold")
...
class EventBridge:
    ...
    This is the Gold equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from gold_constants import get_gold_home
            db_file = get_gold_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("GOLD_HOME", Path.home() / ".gold")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Gold MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'gold[mcp]'"
        )

    mcp = FastMCP(
        "gold",
        instructions=(
            "Gold Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Gold MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
