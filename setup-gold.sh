...
# 5. Symlinks the 'gold' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}◆ Gold Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink gold into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up gold command..."

GOLD_BIN="$SCRIPT_DIR/venv/bin/gold"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$GOLD_BIN" "$COMMAND_LINK_DIR/gold"
echo -e "${GREEN}✓${NC} Symlinked gold → $COMMAND_LINK_DISPLAY_DIR/gold"
...
# ============================================================================
# Seed bundled skills into ~/.gold/skills/
# ============================================================================

GOLD_SKILLS_DIR="${GOLD_HOME:-$HOME/.gold}/skills"
mkdir -p "$GOLD_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.gold/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$GOLD_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  gold status        # Check configuration"
if is_termux; then
    echo "  gold gateway       # Run gateway in foreground"
else
    echo "  gold gateway install # Install gateway service (messaging + cron)"
fi
echo "  gold cron list     # View scheduled jobs"
echo "  gold doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m gold_cli.main setup
fi
