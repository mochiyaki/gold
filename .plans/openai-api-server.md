# OpenAI-Compatible API Server for Gold Agent
...
SSE streaming. By exposing this endpoint, gold-agent becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.gold/.env`
2. Run `gold gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with gold-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns gold-agent as a model) |
...
Authorization: Bearer gold-api-key-here
...
But gold-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "gold-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured gold-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of gold-agent's core prompt
...
Once implemented, gold-agent works as a drop-in backend for:
