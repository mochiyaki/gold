---
name: honcho
description: Configure and use Honcho memory with Gold -- cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation and recall settings.
version: 1.0.0
author: Gold Agent
license: MIT
metadata:
...
# Honcho Memory for Gold

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Gold profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/gold#running-honcho-locally-with-gold

### Verify

```bash
gold honcho status    # shows resolved config, connection test, peer info
```
...
Honcho models conversations as interactions between **peers**. Gold creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Gold instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Gold run |
| `global` | Single session across all directories |

Manual override: `gold honcho map my-project-name`
...
Each Gold profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
gold profile create coder --clone
# creates host block gold.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `gold.coder` host block in `honcho.json`
...
```bash
gold honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Config file: `$GOLD_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `gold honcho setup`. Ensure `memory.provider: honcho` is in `~/.gold/config.yaml`.

### Memory not persisting across sessions
Check `gold honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `gold profile create <name> --clone`. For existing profiles: `gold honcho sync`.
...
| Command | Description |
|---------|-------------|
| `gold honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `gold honcho status` | Show resolved config, connection test, peer info for active profile |
| `gold honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `gold honcho disable` | Disable Honcho for the active profile |
| `gold honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `gold honcho peers` | Show peer identities across all profiles |
| `gold honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `gold honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `gold honcho sessions` | List known directory-to-session-name mappings |
| `gold honcho map <name>` | Map current working directory to a Honcho session name |
| `gold honcho identity` | Seed AI peer identity or show both peer representations |
| `gold honcho sync` | Create host blocks for all Gold profiles that don't have one yet |
| `gold honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Gold + Honcho |
| `gold memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `gold memory status` | Show active memory provider and config |
| `gold memory off` | Disable external memory provider |
