...
- smoke-test a server with the FastMCP CLI before wiring it into Gold or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Gold. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.gold/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.gold/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Gold integration, either:

- configure the server in `~/.gold/config.yaml` using the `native-mcp` skill, or
...
### Gold cannot see the deployed server

The server-building part may be correct while the Gold config is not. Load the `native-mcp` skill and configure the server in `~/.gold/config.yaml`, then restart Gold.
