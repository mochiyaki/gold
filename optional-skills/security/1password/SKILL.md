...
version: 1.0.0
author: arceus77-7, enhanced by Gold Agent
license: MIT
metadata:
  gold:
...
- `tmux` available for stable authenticated sessions during Gold terminal calls (desktop app flow only)
...
### Service Account (recommended for Gold)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.gold/.env` (the skill will prompt for this on first load).
...
## Gold Execution Pattern (desktop app flow)

Gold terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/gold-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/gold-op.sock"
