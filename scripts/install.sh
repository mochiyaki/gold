...
# Configuration
REPO_URL_SSH="git@github.com:mochiyaki/gold.git"
REPO_URL_HTTPS="https://github.com/mochiyaki/gold.git"
GOLD_HOME="$HOME/.gold"
INSTALL_DIR="${GOLD_INSTALL_DIR:-$GOLD_HOME/gold}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.gold/gold)"
...
get_gold_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/gold" ]; then
        echo "$link_dir/gold"
    else
        echo "gold"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/mochiyaki/gold/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$GOLD_HOME/node/bin/node" ]; then
        export PATH="$GOLD_HOME/node/bin:$PATH"
        local found_ver=$("$GOLD_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Gold-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.gold/node/..."
    ...
    # Place into ~/.gold/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$GOLD_HOME/node"
    mkdir -p "$GOLD_HOME"
    mv "$extracted_dir" "$GOLD_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$GOLD_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$GOLD_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$GOLD_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$GOLD_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$GOLD_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.gold/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Gold Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="gold-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Gold behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up gold command..."

    if [ "$USE_VENV" = true ]; then
        GOLD_BIN="$INSTALL_DIR/venv/bin/gold"
    else
        GOLD_BIN="$(which gold 2>/dev/null || echo "")"
        if [ -z "$GOLD_BIN" ]; then
            log_warn "gold not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$GOLD_BIN" ]; then
        log_warn "gold entry point not found at $GOLD_BIN"
    ...
    # Create a user-facing shim for the gold command.
    mkdir -p "$command_link_dir"
    ln -sf "$GOLD_BIN" "$command_link_dir/gold"
    log_success "Symlinked gold → $command_link_display_dir/gold"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "gold command ready"
        return 0
    fi
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Gold Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Gold Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so gold works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.gold directory structure (config at top level, code in subdir)
    mkdir -p "$GOLD_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.gold/.env (top level, easy to find)
    if [ ! -f "$GOLD_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$GOLD_HOME/.env"
            log_success "Created ~/.gold/.env from template"
        else
            touch "$GOLD_HOME/.env"
            log_success "Created ~/.gold/.env"
        fi
    else
        log_info "~/.gold/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.gold/config.yaml (top level, easy to find)
    if [ ! -f "$GOLD_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$GOLD_HOME/config.yaml"
            log_success "Created ~/.gold/config.yaml from template"
        fi
    else
        log_info "~/.gold/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$GOLD_HOME/SOUL.md" ]; then
        cat > "$GOLD_HOME/SOUL.md" << 'SOUL_EOF'
# Gold Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Gold communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.gold/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.gold/"

    # Seed bundled skills into ~/.gold/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.gold/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.gold/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$GOLD_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$GOLD_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.gold/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Gold itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'gold setup' after install."
        return 0
    fi
    ...
    # Run gold setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m gold_cli.main setup < /dev/tty
    else
        python -m gold_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$GOLD_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Gold to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$GOLD_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'gold whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                GOLD_CMD="$(get_gold_command_path)"
                $GOLD_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'gold whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'gold gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $GOLD_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $GOLD_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: gold gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: gold gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $GOLD_CMD gateway > "$GOLD_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.gold/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: gold gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: gold gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.gold/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.gold/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.gold/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.gold/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.gold/gold/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}gold${NC}              Start chatting"
    echo -e "   ${GREEN}gold setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}gold config${NC}       View/edit configuration"
    echo -e "   ${GREEN}gold config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}gold gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}gold update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'gold' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'gold' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
