 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.gold/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.gold', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.gold', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.gold', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.gold', 'audio_cache');
const PAIR_ONLY = args.includes('--pair-only');
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot" or "self-chat"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '◆ *Gold Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Gold Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Gold' own reply messages in self-chat mode to avoid loops.
