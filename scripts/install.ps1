# ============================================================================
# Gold Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/mochiyaki/gold/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$GoldHome = "$env:LOCALAPPDATA\gold",
    [string]$InstallDir = "$env:LOCALAPPDATA\gold\gold"
)
...
$RepoUrlSsh = "git@github.com:mochiyaki/gold.git"
$RepoUrlHttps = "https://github.com/mochiyaki/gold.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             🔸◆ Gold Agent Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Test-Node {
    ...
    $managedNode = "$GoldHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$GoldHome\node;$env:Path"
        Write-Success "Node.js $version found (Gold-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.gold/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\gold-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$GoldHome\node") { Remove-Item -Recurse -Force "$GoldHome\node" }
                Move-Item $extractedDir.FullName "$GoldHome\node"
                $env:Path = "$GoldHome\node;$env:Path"

                $version = & "$GoldHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.gold/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/mochiyaki/gold/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\gold-$Branch.zip"
                $extractPath = "$env:TEMP\gold-extract"
...
function Set-PathVariable {
    Write-Info "Setting up gold command..."
    
    if ($NoVenv) {
        $goldBin = "$InstallDir"
    } else {
        $goldBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so gold is globally available
    # On Windows, the gold.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$goldBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$goldBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $goldBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set GOLD_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\gold instead
    # of the Unix default ~/.gold
    $currentGoldHome = [Environment]::GetEnvironmentVariable("GOLD_HOME", "User")
    if (-not $currentGoldHome -or $currentGoldHome -ne $GoldHome) {
        [Environment]::SetEnvironmentVariable("GOLD_HOME", $GoldHome, "User")
        Write-Success "Set GOLD_HOME=$GoldHome"
    }
    $env:GOLD_HOME = $GoldHome
    
    # Update current session
    $env:Path = "$goldBin;$env:Path"
    
    Write-Success "gold command ready"
}
...
function Copy-ConfigTemplates {
    ...
    New-Item -ItemType Directory -Force -Path "$GoldHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GoldHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GoldHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GoldHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GoldHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GoldHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GoldHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GoldHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GoldHome\skills" | Out-Null
    New-Item -ItemType Directory -Force -Path "$GoldHome\whatsapp\session" | Out-Null
    
    # Create .env
    $envPath = "$GoldHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.gold/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.gold/.env"
        }
    } else {
        Write-Info "~/.gold/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$GoldHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.gold/config.yaml from template"
        }
    } else {
        Write-Info "~/.gold/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$GoldHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.gold/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.gold/"
    
    # Seed bundled skills into ~/.gold/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.gold/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.gold/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$GoldHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.gold/skills/"
            }
        }
    }
}
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m gold_cli.main setup
    } else {
        python -m gold_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $goldCmd = "$InstallDir\venv\Scripts\gold.exe"
    if (-not (Test-Path $goldCmd)) {
        $goldCmd = "gold"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$GoldHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'gold whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$GoldHome\logs\gateway.log"
            Start-Process -FilePath $goldCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$GoldHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: gold gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: gold gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$GoldHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$GoldHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$GoldHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$GoldHome\gold\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   gold              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   gold setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   gold config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   gold config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   gold gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   gold update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/mochiyaki/gold/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
