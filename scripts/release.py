...
VERSION_FILE = REPO_ROOT / "gold_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "teknium@mochiyaki.com": "teknium1",
    ...
    "shannon@mochiyaki.com": "shannonsands",
    ...
    "emozilla@mochiyaki.com": "emozilla",
    ...
    "jonny@mochiyaki.com": "jquesnelle",
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "gold@mochiyaki.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/mochiyaki/gold",
    ...
    lines.append(f"# Gold Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Gold Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Gold Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Gold Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Gold Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Gold Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Gold Agent v{new_version} ({calver_date})' "
