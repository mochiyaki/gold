"""ACP session manager — maps ACP sessions to Gold AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.gold/state.db``) so they
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Gold agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Gold AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Gold runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.gold/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``GOLD_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_gold_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=gold_home / "state.db")
            return self._db_instance
        except Exception:
