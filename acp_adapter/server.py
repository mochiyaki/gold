"""ACP agent server — exposes Gold Agent via the Agent Client Protocol."""
...
try:
    from gold_cli import __version__ as GOLD_VERSION
except Exception:
    GOLD_VERSION = "0.0.0"
...
class GoldACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Gold AIAgent."""
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = getattr(state.agent, "enabled_toolsets", None) or ["gold-acp"]
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Gold using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="gold", version=GOLD_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Gold on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from gold_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            from model_tools import get_tool_definitions
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["gold-acp"]
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Gold Agent v{GOLD_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Gold has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
