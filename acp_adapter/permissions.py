"""ACP permission bridging — maps ACP approval requests to gold approval callbacks."""
...
# Maps ACP PermissionOptionKind -> gold approval result strings
_KIND_TO_GOLD = {
    "allow_once": "once",
    "allow_always": "always",
    "reject_once": "deny",
...
def make_approval_callback(
    ...
    """
    Return a gold-compatible ``approval_callback(command, description) -> str``
        ...
        if isinstance(outcome, AllowedOutcome):
            ...
            for opt in options:
                if opt.option_id == option_id:
                    return _KIND_TO_GOLD.get(opt.kind, "deny")
