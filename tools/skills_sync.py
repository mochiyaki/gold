...
Copies bundled skills from the repo's skills/ directory into ~/.gold/skills/
...
The manifest lives at ~/.gold/skills/.bundled_manifest.
"""
...
def _get_bundled_dir() -> Path:
    ...
    Checks GOLD_BUNDLED_SKILLS env var first (set by Nix wrapper),
    then falls back to the relative path from this source file.
    """
    env_override = os.getenv("GOLD_BUNDLED_SKILLS")
    if env_override:
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.gold/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.gold/skills/ using the manifest.
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.gold/skills/ ...")
