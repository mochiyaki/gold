...
Architecture:
    ~/.gold/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        GOLD_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = get_gold_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("GOLD_CHECKPOINT_TIMEOUT", "30"))))
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "gold@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "Gold Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "GOLD_WORKDIR").write_text(
