...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "gold-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.gold"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.gold directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.gold"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="gold-ssh-bulk-") as staging:
