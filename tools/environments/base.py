"""Base class for all Gold execution environment backends.
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {GOLD_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_gold_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__GOLD_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Gold backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/gold-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/gold-cwd-{self._session_id}.txt"
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"GOLD_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __GOLD_CWD_{session}__ marker from stdout output.
