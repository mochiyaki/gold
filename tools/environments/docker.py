...
from tools.environments.local import _GOLD_PROVIDER_ENV_BLOCKLIST
...
def _load_gold_env_vars() -> dict[str, str]:
    """Load ~/.gold/.env values without failing Docker command execution."""
    try:
        from gold_cli.config import load_env
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``GOLD_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("GOLD_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using GOLD_DOCKER_BINARY override: %s", override)
        return override
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.gold/sandboxes/). Non-persistent
        ...
        container_name = f"gold-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Gold secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _GOLD_PROVIDER_ENV_BLOCKLIST)
        gold_env = _load_gold_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = gold_env.get(key)
