...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/gold-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/gold-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/gold-results"
HEREDOC_MARKER = "GOLD_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/gold-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"GOLD_PERSIST_{uuid.uuid4().hex[:8]}"
