...
LOCAL_STT_COMMAND_ENV = "GOLD_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "GOLD_LOCAL_STT_LANGUAGE"
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set GOLD_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="gold-local-stt-") as output_dir:
