...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("GOLD_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $GOLD_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_GOLD_ENV_PATH = (
    r'(?:~\/\.gold/|'
    r'(?:\$home|\$\{home\})/\.gold/|'
    r'(?:\$gold_home|\$\{gold_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bgold\s+gateway\s+(stop|restart)\b', "stop/restart gold gateway (kills running agents)"),
    (r'\bgold\s+update\b', "gold update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart gold-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart gold-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(gold|gateway|cli\.py)\b', "kill gold/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill gold` but not
    # `kill -9 $(pgrep -f gold)` because the substitution is opaque
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["GOLD_SPINNER_PAUSE"] = "1"
    try:
        while True:
            print()
        ...
        if "GOLD_SPINNER_PAUSE" in os.environ:
            del os.environ["GOLD_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("GOLD_YOLO_MODE") or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("GOLD_INTERACTIVE")
    is_gateway = os.getenv("GOLD_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("GOLD_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("GOLD_YOLO_MODE") or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("GOLD_INTERACTIVE")
    is_gateway = os.getenv("GOLD_GATEWAY_SESSION")
    is_ask = os.getenv("GOLD_EXEC_ASK")
