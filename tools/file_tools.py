...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Gold internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files.
        from gold_constants import get_gold_home as _get_hh
        _gold_home = _get_hh().resolve()
        _blocked_dirs = [
            _gold_home / "skills" / ".hub" / "index-cache",
            _gold_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps({
                    "error": (
                        f"Access denied: {path} is an internal Gold cache file "
