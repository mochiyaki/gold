...
# Path to tinker-atropos submodule (relative to gold root)
GOLD_ROOT = Path(__file__).parent.parent
TINKER_ATROPOS_ROOT = GOLD_ROOT / "tinker-atropos"
ENVIRONMENTS_DIR = TINKER_ATROPOS_ROOT / "tinker_atropos" / "environments"
CONFIGS_DIR = TINKER_ATROPOS_ROOT / "configs"
LOGS_DIR = get_gold_home() / "logs" / "rl_training"
...
async def rl_start_training() -> str:
    ...
    if not os.getenv("TINKER_API_KEY"):
        return json.dumps({
            "error": "TINKER_API_KEY not set. Add it to ~/.gold/.env",
        }, indent=2)
...
async def rl_check_status(run_id: str) -> str:
    ...
    try:
        ...
        api = wandb.Api()
        runs = api.runs(
            f"{os.getenv('WANDB_ENTITY', 'mochiyaki')}/{run_state.wandb_project}",
