...
Configuration is loaded from ~/.gold/config.yaml under the 'tts:' key.
...
def _get_default_output_dir() -> str:
    ...
    return str(get_gold_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.gold/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.gold/config.yaml.
    ...
    try:
        ...
        logger.debug("gold_cli.config not available, using default TTS config")
        return {}
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.gold/config.yaml (tts: section).
    ...
    platform = get_session_env("GOLD_SESSION_PLATFORM", "").lower()
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run gold setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": "Optional custom file path to save the audio. Defaults to ~/.gold/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
