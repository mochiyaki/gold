"""
Cron job management tools for Gold Agent.
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("GOLD_SESSION_PLATFORM")
    origin_chat_id = get_session_env("GOLD_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        thread_id = get_session_env("GOLD_SESSION_THREAD_ID") or None
        if thread_id:
            logger.debug(
        ...
        return {
            ...
            "chat_name": get_session_env("GOLD_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
        }
    return None
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via gold model.
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within GOLD_HOME/scripts/.
    ...
    # Only relative paths within ~/.gold/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.gold/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.gold/scripts/ and use just the filename."
        )
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": "Optional path to a Python script that runs before each cron job execution. Its stdout is injected into the prompt as context. Use for data collection and change detection. Relative paths resolve under ~/.gold/scripts/. On update, pass empty string to clear."
            },
        },
        "required": ["action"]
    }
...
def check_cronjob_requirements() -> bool:
    ...
    """
    return bool(
        os.getenv("GOLD_INTERACTIVE")
        or os.getenv("GOLD_GATEWAY_SESSION")
        or os.getenv("GOLD_EXEC_ASK")
    )
