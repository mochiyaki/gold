...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    gold_node_bin = str(gold_home / "node" / "bin")
    return [gold_node_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-gold_…`` the resulting socket path exceeds the
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Gold-managed
    ...
    # Build an extended search PATH including Gold-managed Node, macOS
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = gold_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    screenshots_dir = get_gold_dir("cache/screenshots", "browser_screenshots")
