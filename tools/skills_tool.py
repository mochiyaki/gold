...
# All skills live in ~/.gold/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from GOLD_HOME/.env."""
    env_path = get_gold_home() / ".env"
...
def _is_gateway_surface() -> bool:
    if os.getenv("GOLD_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("GOLD_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return "Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to ~/.gold/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.gold/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("GOLD_PLATFORM")
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.gold/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``gold skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": "No skills found. Skills directory created at ~/.gold/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    from gold_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: gold plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
...
def skill_view(name: str, file_path: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        if ":" in name:
            ...
            from gold_cli.plugins import discover_plugins, get_plugin_manager
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.gold/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `gold skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.gold.* first (agentskills.io convention), fall back to top-level
        gold_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            gold_meta = metadata.get("gold", {}) or {}

        tags = _parse_tags(gold_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            gold_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
