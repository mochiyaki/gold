...
discovers their tools, and registers them into the gold tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.gold/config.yaml under the ``mcp_servers`` key.
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            gold_home = os.path.expanduser(
                os.getenv(
                    "GOLD_HOME", os.path.join(os.path.expanduser("~"), ".gold")
                )
            )
            candidates = [
                os.path.join(gold_home, "node", "bin", resolved_command),
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Gold config file.
    ...
    ``os.environ`` (which includes ``~/.gold/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Gold's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Gold registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``gold tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Gold registry.
