...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Gold exit
...
# display_gold_home imported lazily at call site (stale-module safety during gold update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of gold directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "gold-*")):
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (GOLD_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (GOLD_INTERACTIVE=1):
    ...
    if not has_configured_password and not sudo_password and os.getenv("GOLD_INTERACTIVE"):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.gold/.env or environment variables."
        )
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "GOLD_ENABLE_NOUS_MANAGED_TOOLS is not enabled and no direct "
        ...
        if background:
            ...
            try:
                ...
                if notify_on_complete and background:
                    ...
                    _gw_platform = _gse("GOLD_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("GOLD_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("GOLD_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("GOLD_SESSION_USER_ID", "")
                        _gw_user_name = _gse("GOLD_SESSION_USER_NAME", "")
