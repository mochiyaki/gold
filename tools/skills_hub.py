#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Gold Skills Hub.
...
Used by gold_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            gold_meta = metadata.get("gold", {})
            if isinstance(gold_meta, dict):
                tags = gold_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.gold/skills/ during setup.  They are discoverable via the Skills Hub
...
# ---------------------------------------------------------------------------
# Gold centralized index source
# ---------------------------------------------------------------------------

GOLD_INDEX_URL = "https://gold.mochiyaki.com/docs/api/skills-index.json"
GOLD_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "gold-index.json"
GOLD_INDEX_TTL = 6 * 3600  # 6 hours


def _load_gold_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for GOLD_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if GOLD_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - GOLD_INDEX_CACHE_FILE.stat().st_mtime
            if age < GOLD_INDEX_TTL:
                return json.loads(GOLD_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(GOLD_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Gold index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Gold index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        GOLD_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        GOLD_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class GoldIndexSource(SkillSource):
    """Skill source backed by the centralized Gold Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "gold-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "gold-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        GoldIndexSource(auth=auth), # Centralized index (search + resolved install paths)
