...
Lets the LLM write a Python script that calls Gold tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `gold_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `gold_tools.py` with file-based RPC stubs
...
def generate_gold_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the gold_tools.py stub module.
...
"""Auto-generated Gold tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["GOLD_RPC_SOCKET"])
...
"""Auto-generated Gold tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, time

_RPC_DIR = os.environ.get("GOLD_RPC_DIR") or os.path.join(tempfile.gettempdir(), "gold_rpc")
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    The script and the generated gold_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/gold_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_gold_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/gold_tools.py", tools_src)
        ...
        env_prefix = (
            f"GOLD_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("GOLD_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with gold_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="gold_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"gold_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated gold_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_gold_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "gold_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["GOLD_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the gold root is importable in the sandbox so
        # repo-root modules are available to child scripts.
        _gold_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        child_env["PYTHONPATH"] = _gold_root + (os.pathsep + _existing_pp if _existing_pp else "")
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.
        _tz_name = os.getenv("GOLD_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name

        # Per-profile HOME isolation: redirect system tool configs into
        # {GOLD_HOME}/home/ when that directory exists.
        from gold_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.gold/.env')).
...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    ...
    When tools are disabled via ``gold tools`` (e.g. web is turned off),
    ...
    description = (
        "Run a Python script that can call Gold tools programmatically. "
        ...
        f"Available via `from gold_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        "Limits: 5-minute timeout, 50KB stdout cap, max 50 tool calls per script. "
        "terminal() is foreground-only (no background or pty).\n\n"
        "Print your final result to stdout. Use Python stdlib (json, re, math, csv, "
        "datetime, collections, etc.) for processing between tool calls.\n\n"
        "Also available (no import needed — built into gold_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from gold_tools import {import_str}` "
