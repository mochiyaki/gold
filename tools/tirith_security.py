...
it is automatically downloaded from GitHub releases to $GOLD_HOME/bin/tirith.
...
def _get_gold_home() -> str:
    """Return the Gold home directory, respecting GOLD_HOME env var."""
    return str(get_gold_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_gold_home(), ".tirith-install-failed")
...
def _gold_bin_dir() -> str:
    """Return $GOLD_HOME/bin, creating it if needed."""
    d = os.path.join(_get_gold_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $GOLD_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_gold_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $GOLD_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $GOLD_HOME/bin/tirith
    ...
    gold_bin = os.path.join(_gold_bin_dir(), "tirith")
    if os.path.isfile(gold_bin) and os.access(gold_bin, os.X_OK):
        _resolved_path = gold_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return gold_bin
