...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.gold/.env``.
"""
...
def _managed_persistence_enabled() -> bool:
    """Return whether Gold-managed persistence is enabled for Camofox.
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the Gold profile so the Camofox server can map it
    ...
    with _sessions_lock:
        ...
        if _managed_persistence_enabled():
            ...
            session = {
                "user_id": f"gold_{uuid.uuid4().hex[:10]}",
                "tab_id": None,
...
def camofox_vision(question: str, annotate: bool = False,
    ...
    try:
        ...
        screenshots_dir = get_gold_home() / "browser_screenshots"
