...
def _resolve_gold_home() -> Path:
    from gold_constants import get_gold_home
    return get_gold_home()


def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.gold",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``GOLD_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside GOLD_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    gold_home = _resolve_gold_home()

    # Reject absolute paths — they bypass the GOLD_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to GOLD_HOME)",
            relative_path,
        )
        return False

    host_path = gold_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape GOLD_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, gold_home)
...
def _load_config_files() -> List[Dict[str, str]]:
    ...
    try:
        from gold_cli.config import read_raw_config
        gold_home = _resolve_gold_home()
        cfg = read_raw_config()
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.gold/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="gold-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching gold_constants.get_gold_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_gold_dir()`` for backward compatibility with old directory layouts.
    """
    from gold_constants import get_gold_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_gold_dir(new_subpath, old_name)
        if host_dir.is_dir():
