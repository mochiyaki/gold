...
Backend is selected during ``gold tools`` setup (web.backend in config.yaml).
When available, Gold can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.gold/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``gold tools``).
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if managed_nous_tools_enabled():
        message += (
            ...
            "(`gold model`) and provide FIRECRAWL_GATEWAY_URL or TOOL_GATEWAY_DOMAIN."
        )
...
def _get_firecrawl_client():
    ...
    Gold falls back to the Firecrawl tool-gateway for logged-in Nous Subscribers.
    """
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    ...
    return host == "mochiyaki.com" or host.endswith(".mochiyaki.com")
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        ...
        extra_body = get_auxiliary_extra_body() or {"tags": ["product=gold"]}
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "gold"
    return _exa_client
