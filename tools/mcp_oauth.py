...
This module provides the glue:
    - ``GoldTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Gold Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses GOLD_HOME so each profile gets its own OAuth tokens.
    Layout: ``GOLD_HOME/mcp-tokens/``
    """
    try:
        from gold_constants import get_gold_home
        base = Path(get_gold_home())
    except ImportError:
        base = Path(os.environ.get("GOLD_HOME", str(Path.home() / ".gold")))
...
# ---------------------------------------------------------------------------
# GoldTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class GoldTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        GOLD_HOME/mcp-tokens/<server_name>.json         -- tokens
        GOLD_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Gold.</p></body></html>"
            ) if code else (
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = GoldTokenStorage(server_name)
    storage.remove()
...
def build_oauth_auth(
    ...
    # --- Client metadata ---
    client_name = cfg.get("client_name", "Gold Agent")
    scope = cfg.get("scope")
