...
from gold_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``GOLD_PLATFORM`` or
            ``GOLD_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.gold/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.gold/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(gold, dict):
        gold = {}
    return {
        "fallback_for_toolsets": gold.get("fallback_for_toolsets", []),
        "requires_toolsets": gold.get("requires_toolsets", []),
        "fallback_for_tools": gold.get("fallback_for_tools", []),
        "requires_tools": gold.get("requires_tools", []),
    }
