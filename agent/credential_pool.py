...
import gold_cli.auth as auth_mod
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_cli(self, entry: PooledCredential) -> PooledCredential:
        ...
        When the Codex CLI (or another Gold profile) refreshes its token,
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("gold_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # The Codex CLI (or another Gold profile) may have already
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("gold_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Gold profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # the Codex CLI or another Gold profile.
            if (self.provider == "openai-codex"
                    and entry.last_status == STATUS_EXHAUSTED
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Gold PKCE)
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_gold_oauth_credentials

        for source_name, creds in (
            ("gold_pkce", read_gold_oauth_credentials()),
            ("claude_code", read_claude_code_credentials()),
        ):
            if creds and creds.get("accessToken"):
                # Check if user explicitly removed this source
                try:
                    from gold_cli.auth import is_source_suppressed
        ...
        # Gold auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from gold_cli.auth import resolve_qwen_runtime_credentials
        ...
        # Fallback: import from Codex CLI (~/.codex/auth.json) if Gold auth
        ...
        if not (isinstance(tokens, dict) and tokens.get("access_token")):
            try:
                from gold_cli.auth import _import_codex_cli_tokens, _save_codex_tokens
                cli_tokens = _import_codex_cli_tokens()
                if cli_tokens:
                    logger.info("Importing Codex CLI tokens into Gold auth store.")
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "gold_pkce"}
        )
    ]
