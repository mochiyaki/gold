...
_SENSITIVE_GOLD_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    gold_home = get_gold_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(gold_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(gold_home / rel for rel in _SENSITIVE_GOLD_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Gold path and cannot be attached")
