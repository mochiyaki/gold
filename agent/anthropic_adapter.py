"""Anthropic Messages API adapter for Gold Agent.

Translates between Gold's internal OpenAI-style message format and
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Gold historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Gold)
    ...
    # 1. Gold-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Gold configs.
...
# ── Gold-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.gold/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_GOLD_OAUTH_FILE = get_gold_home() / ".anthropic_oauth.json"
...
def run_gold_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Gold-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize Gold with your Claude Pro/Max subscription.")
    print()
...
def read_gold_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Gold-managed OAuth credentials from ~/.gold/.anthropic_oauth.json."""
    if _GOLD_OAUTH_FILE.exists():
        try:
            data = json.loads(_GOLD_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Gold OAuth credentials: %s", e)
    return None
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Gold Agent", "Claude Code")
                text = text.replace("Gold agent", "Claude Code")
                text = text.replace("gold", "claude-code")
