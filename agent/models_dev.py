...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.gold/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Gold ↔ models.dev
# ---------------------------------------------------------------------------

# Gold provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Gold (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_gold_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Gold provider ID to its models dict from models.dev.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Gold provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Gold ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Gold or models.dev provider ID.  Tries exact match then
