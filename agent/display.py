...
# Shimmer palette: 12 brightness stops from dim → bright → dim (neutral gray-gold)
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in Gold' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("GOLD_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
            t = time.time()
