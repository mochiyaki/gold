...
Resolution order for text tasks (auto mode):
  ...
  2. Nous Portal (~/.gold/auth.json active provider)
...
NOUS_EXTRA_BODY = {"tags": ["product=gold"]}
...
_NOUS_DEFAULT_BASE_URL = "https://inference-api.mochiyaki.com/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_gold_home() / "auth.json"
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.gold/auth.json for an active Nous provider.
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Gold auth store.
    ...
    try:
        from gold_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from gold_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from gold_cli.auth import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from gold_cli.models import check_nous_free_tier
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `gold model` but the
    #    old OPENAI_BASE_URL lingers in ~/.gold/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: gold model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.gold/.env",
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: gold auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: gold model)")
                return None, None
    ...
    try:
        from gold_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("gold_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from gold_cli.models import _should_use_copilot_responses_api
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=gold"])
    if merged_extra:
