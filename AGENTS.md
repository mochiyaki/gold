...
├── toolsets.py           # Toolset definitions, _GOLD_CORE_TOOLS list
├── cli.py                # GoldCLI class — interactive CLI orchestrator
├── gold_state.py       # SessionDB — SQLite session store (FTS5 search)
...
├── gold_cli/           # CLI subcommands and setup
│   ├── main.py           # Entry point — all `gold` subcommands
│   ├── config.py         # DEFAULT_CONFIG, OPTIONAL_ENV_VARS, migration
│   ├── commands.py       # Slash command definitions + SlashCommandCompleter
│   ├── callbacks.py      # Terminal callbacks (clarify, sudo, approval)
│   ├── setup.py          # Interactive setup wizard
│   ├── skin_engine.py    # Skin/theme engine — CLI visual customization
│   ├── skills_config.py  # `gold skills` — enable/disable skills per platform
│   ├── tools_config.py   # `gold tools` — enable/disable tools per platform
...
- **Skin engine** (`gold_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `GoldCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
...
- **Slack** — `slack_subcommand_map()` generates `/gold` subcommand routing
...
**2. Add to `toolsets.py`** — either `_GOLD_CORE_TOOLS` (all platforms) or a new toolset.
...
**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_gold_home()` for the base directory — never `Path.home() / ".gold"`. This ensures each profile gets its own state.
...
The skin engine (`gold_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.
...
Gold supports **profiles** — multiple fully isolated instances, each with its own
...
3. **Module-level constants are fine** — they cache `get_gold_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_gold_home()`,
   not `Path.home() / ".gold"`.

4. **Tests that mock `Path.home()` must also set `GOLD_HOME`** — since code now uses
...
6. **Profile operations are HOME-anchored, not GOLD_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".gold" / "profiles"`, NOT `get_gold_home() / "profiles"`.
   This is intentional — it lets `gold -p coder profile list` see all profiles regardless
...
for user-facing print/log messages. Hardcoding `~/.gold` breaks profiles — each profile
has its own `GOLD_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT use `simple_term_menu` for interactive menus
Rendering bugs in tmux/iTerm2 — ghosting on scroll. Use `curses` (stdlib) instead. See `gold_cli/tools_config.py` for the pattern.
