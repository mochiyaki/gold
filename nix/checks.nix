...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      gold = inputs.self.packages.${system}.default;
      ...
      configKeys = pkgs.runCommand "gold-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${goldVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        package-contents = pkgs.runCommand "gold-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${gold}/bin/gold || (echo "FAIL: gold binary missing"; exit 1)
          test -x ${gold}/bin/gold || (echo "FAIL: gold binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${gold}/bin/gold version 2>&1 | grep -qi "gold" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "gold-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in gold gold gold-acp; do
            test -x ${gold}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "gold-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking gold --help ==="
          ${gold}/bin/gold --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${gold}/bin/gold --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "gold-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${gold}/share/gold/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${gold}/share/gold/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "GOLD_BUNDLED_SKILLS" ${gold}/bin/gold || \
            (echo "FAIL: GOLD_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: GOLD_BUNDLED_SKILLS set in wrapper"
        ...
        # Verify GOLD_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "gold-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(GOLD_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking GOLD_MANAGED guards ==="
          check_blocked "config set" ${gold}/bin/gold config set model foo
          check_blocked "config edit" ${gold}/bin/gold config edit
        ...
        in pkgs.runCommand "gold-config-roundtrip" {
          ...
          merge_and_load() {
            local gold_home="$1"
            export GOLD_HOME="$gold_home"
            ${configMergeScript} ${nixSettings} "$gold_home/config.yaml"
            ${goldVenv}/bin/python3 -c '
import json, sys
