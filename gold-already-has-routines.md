# Gold Agent Has Had "Routines" Since March
...
Gold equivalent — works today:
```bash
gold cron create "0 2 * * *" \
...
Gold equivalent — works today:
```bash
gold webhook subscribe auth-watch \
  --events "pull_request" \
...
Gold equivalent — works today:
```bash
gold webhook subscribe alert-triage \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Gold implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Gold Agent |
|---|---|---|
...
## Things Gold Does That Routines Can't
...
Gold has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Gold Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
gold gateway setup    # enable webhooks
gold webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [gold.mochiyaki.com/docs/guides/automation-templates](https://gold.mochiyaki.com/docs/guides/automation-templates)

Documentation: [gold.mochiyaki.com](https://gold.mochiyaki.com)

GitHub: [github.com/mochiyaki/gold](https://github.com/mochiyaki/gold)

---

*Gold Agent is built by [Nous Research](https://mochiyaki.com). Open source, model-agnostic, runs on your infrastructure.*
