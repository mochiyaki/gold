# Gold Atropos Environments

This directory contains the integration layer between **gold's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  GoldAgentBaseEnv    │  gold_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     GoldSweEnv    TerminalBench2EvalEnv
...
**GoldAgentBaseEnv** (`gold_base_env.py`) extends BaseEnv with gold specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves gold toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `GoldAgentBaseEnv` and implement:
...
`GoldAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as gold's `run_agent.py`:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** gold tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any gold tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some gold tools use `asyncio.run()` internally (e.g., the Modal backend). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Available parsers:
- `gold` -- Gold/ChatML `<tool_call>` XML format
...
├── gold_base_env.py            # Abstract base (GoldAgentBaseEnv)
├── agent_loop.py                 # Multi-turn agent engine (GoldAgentLoop)
...
│
├── gold_swe_env/               # SWE-bench style training environment
│   └── gold_swe_env.py
│
...
### GoldSweEnv (`gold_swe_env/`)

SWE-bench style training environment. The model gets a coding task, uses terminal + file + web tools to solve it, and the reward function runs tests in the same Modal sandbox.

```bash
python environments/gold_swe_env/gold_swe_env.py serve \
...
2. Create your env file inheriting from `GoldAgentBaseEnv`
3. Implement the four abstract methods + `evaluate()`

```python
from environments.gold_base_env import GoldAgentBaseEnv, GoldAgentEnvConfig

class MyEnvConfig(GoldAgentEnvConfig):
    pass  # Add custom fields as needed

class MyEnv(GoldAgentBaseEnv):
    name = "my-env"
...
2. Inherit from `GoldAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which gold toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `gold` |
