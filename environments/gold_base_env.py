"""
GoldAgentBaseEnv -- Abstract Base Environment for Gold + Atropos

Provides the Atropos integration plumbing that all gold environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via GoldAgentLoop
...
# Ensure the gold repo root is on sys.path so that imports like
...
# Load API keys from gold/.env so all environments can access them
...
from environments.agent_loop import AgentResult, GoldAgentLoop
...
# Import gold toolset infrastructure
...
class GoldAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for gold Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of gold toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: gold, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/gold-results/ "
...
class GoldAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for gold Atropos integration.
    ...
    env_config_cls = GoldAgentEnvConfig

    def __init__(
        self,
        config: GoldAgentEnvConfig,
        ...
        # Set terminal environment variables so gold tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY gold tool (terminal, file, web,
