...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs GoldAgentLoop with terminal tool only
...
Requires: pip install gold[yc-bench]
"""
...
class YCBenchEvalConfig(GoldAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends GoldAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(GoldAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The GoldAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs GoldAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our GoldAgentLoop.
