#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Gold Agent.
...
try:
    from gold_constants import display_gold_home, get_gold_home
except ModuleNotFoundError:
    GOLD_AGENT_ROOT = Path(__file__).resolve().parents[4]
    if GOLD_AGENT_ROOT.exists():
        sys.path.insert(0, str(GOLD_AGENT_ROOT))
    from gold_constants import display_gold_home, get_gold_home

GOLD_HOME = get_gold_home()
TOKEN_PATH = GOLD_HOME / "google_token.json"
CLIENT_SECRET_PATH = GOLD_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = GOLD_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Gold profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Gold home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_gold_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Gold")
