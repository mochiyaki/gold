...
## 5. GODMODE FAST — Gold 4 405B (Zero Refusal)

**Model:** `mochiyaki/gold-4-405b`
**Strategy:** Gold models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Gold Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.gold/config.yaml`:
