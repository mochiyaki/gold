...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.gold/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("GOLD_HOME", Path.home() / ".gold")) / "skills" / "red-teaming" / "godmode"
...
GOLD_HOME = Path(os.getenv("GOLD_HOME", Path.home() / ".gold"))
CONFIG_PATH = GOLD_HOME / "config.yaml"
PREFILL_PATH = GOLD_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Gold models — already uncensored, just needs clean prompt
    "gold": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "gold" in model_lower or "nous" in model_lower:
        return "gold"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Gold config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.gold/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Gold for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Gold for changes to take effect.")
