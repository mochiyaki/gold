...
version: 1.0.0
author: Siddharth Balyan + Gold Agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into Gold. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.gold/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.gold/.env ~/.config/x-cli/.env
```
...
x-cli tweet search "from:mochiyaki" --max 3
...
- **Credential drift**: if you rotate tokens in `~/.gold/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
