...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.gold/.env ] && grep -q "^GITHUB_TOKEN=" ~/.gold/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.gold/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Gold Agent\",
    \"comments\": [
...
```bash
source ~/.gold/skills/github/github-auth/scripts/gh-env.sh
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Gold Agent. Code looks clean — good test coverage, no security concerns."
