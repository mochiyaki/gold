...
- Give it a title like "gold-<machine-name>"
...
elif [ -f ~/.gold/.env ] && grep -q "^GITHUB_TOKEN=" ~/.gold/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.gold/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
