# Migrating from OpenClaw to Gold Agent

This guide covers how to import your OpenClaw settings, memories, skills, and API keys into Gold Agent.

## Three Ways to Migrate

### 1. Automatic (during first-time setup)

When you run `gold setup` for the first time and Gold detects `~/.openclaw`, it automatically offers to import your OpenClaw data before configuration begins. Just accept the prompt and everything is handled for you.

### 2. CLI Command (quick, scriptable)

```bash
gold claw migrate                      # Preview then migrate (always shows preview first)
gold claw migrate --dry-run            # Preview only, no changes
gold claw migrate --preset user-data   # Migrate without API keys/secrets
gold claw migrate --yes                # Skip confirmation prompt
```
...
```
> Migrate my OpenClaw setup to Gold
```
...
|------|--------|-------------|
| SOUL.md | `~/.openclaw/workspace/SOUL.md` | `~/.gold/SOUL.md` |
| Memory entries | `~/.openclaw/workspace/MEMORY.md` | `~/.gold/memories/MEMORY.md` |
| User profile | `~/.openclaw/workspace/USER.md` | `~/.gold/memories/USER.md` |
| Skills | `~/.openclaw/workspace/skills/` | `~/.gold/skills/openclaw-imports/` |
| Command allowlist | `~/.openclaw/workspace/exec_approval_patterns.yaml` | Merged into `~/.gold/config.yaml` |
| Messaging settings | `~/.openclaw/config.yaml` (TELEGRAM_ALLOWED_USERS, MESSAGING_CWD) | `~/.gold/.env` |
| TTS assets | `~/.openclaw/workspace/tts/` | `~/.gold/tts/` |
...
| Item | Source | Destination |
|------|--------|-------------|
| Telegram bot token | `openclaw.json` channels config | `~/.gold/.env` |
| OpenRouter API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.gold/.env` |
| OpenAI API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.gold/.env` |
| Anthropic API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.gold/.env` |
| ElevenLabs API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.gold/.env` |
...
- **TTS provider**: OpenClaw renamed "edge" to "microsoft" — both are recognized and mapped to Gold' "edge"
...
By default, the migration **will not overwrite** existing Gold data:

- **SOUL.md** — skipped if one already exists in `~/.gold/`
...
- **API keys** — skipped if the key is already set in `~/.gold/.env`
...
For executed migrations, the full report is saved to `~/.gold/migration/openclaw/<timestamp>/`.

## Post-Migration Notes

- **Skills require a new session** — imported skills take effect after restarting your agent or starting a new chat.
- **WhatsApp requires re-pairing** — WhatsApp uses QR-code pairing, not token-based auth. Run `gold whatsapp` to pair.
- **Archive cleanup** — after migration, you'll be offered to rename `~/.openclaw/` to `.openclaw.pre-migration/` to prevent state confusion. You can also run `gold claw cleanup` later.
...
```bash
gold claw migrate --source /path/to/.openclaw
```

### "Migration script not found"
The migration script ships with Gold Agent. If you installed via pip (not git clone), the `optional-skills/` directory may not be present. Install the skill from the Skills Hub:
```bash
gold skills install openclaw-migration
```

### Memory overflow
If your OpenClaw MEMORY.md or USER.md exceeds Gold' character limits, excess entries are exported to an overflow file in the migration report directory. You can manually review and add the most important ones.
...
The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `gold config set`.
