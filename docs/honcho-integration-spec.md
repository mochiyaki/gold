...
Comparison of Gold Agent vs. openclaw-honcho — and a porting spec for bringing Gold patterns into other Honcho integrations.

---

## Overview

Two independent Honcho integrations have been built for two different agent runtimes: **Gold Agent** (Python, baked into the runner) and **openclaw-honcho** (TypeScript plugin via hook/tool API). Both use the same Honcho peer paradigm — dual peer model, `session.context()`, `peer.chat()` — but they made different tradeoffs at every layer.

This document maps those tradeoffs and defines a porting spec: a set of Gold-originated patterns, each stated as an integration-agnostic interface, that any Honcho integration can adopt regardless of runtime or language.

> **Scope** Both integrations work correctly today. This spec is about the delta — patterns in Gold that are worth propagating and patterns in openclaw-honcho that Gold should eventually adopt. The spec is additive, not prescriptive.

---

## Architecture comparison

### Gold: baked-in runner
...
| Dimension | Gold Agent | openclaw-honcho |
|---|---|---|
...
| **AI peer name in identity** | Replaces "Gold Agent" in DEFAULT_AGENT_IDENTITY when configured. | Not implemented. |
...
Six patterns from Gold are worth adopting in any Honcho integration. Each is described as an integration-agnostic interface.
...
**openclaw-honcho contributes back (Gold should adopt):**
...
- [ ] **honcho mode / sessions / map** — CLI parity with Gold
...
Greenfield integration. Start from openclaw-honcho's architecture and apply all Gold patterns from day one.
