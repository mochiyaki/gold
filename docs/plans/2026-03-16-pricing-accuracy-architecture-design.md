...
Gold should only show dollar costs when they are backed by an official source for the user's actual billing path.
...
Current Gold behavior has four structural issues:
...
Gold must stop keying pricing solely by `model`.
...
- manual `gold pricing sync`

## Reconciliation Architecture

Live requests may produce only an estimate initially. Gold should reconcile them later when a provider exposes actual billed cost.

Suggested flow:

1. Agent call completes.
2. Gold stores canonical usage plus reconciliation ids.
3. Gold computes an immediate estimate if a pricing source exists.
...
That path cannot satisfy the product requirement. Gold should instead migrate to:
...
This is the minimum architecture that makes the statement "Gold pricing is backed by official sources where possible, and otherwise clearly labeled" defensible.
