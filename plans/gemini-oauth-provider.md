...
## Token Lifecycle
- Store at `~/.gold/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `gold_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `gold_cli/models.py` — Add Gemini model catalog
4. `gold_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `gold_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `gold_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- OR: accept user-provided client_id via GOLD_GEMINI_CLIENT_ID env var
...
- gold Copilot OAuth: `gold_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
